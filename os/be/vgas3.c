...
s3init(Mode* mode)
{
	...
	/*
	 * Set linear aperture, just use 2Mb on a 2Mb boundary for now.
	 */
	if((l = getspace(2*1024*1024, 2*1024*1024)) == 0)
		return;
	 ...
	 */
	l = PADDR(l)>>16;
	vgaxo(Crtx, 0x59, (l>>8) & 0xFF);
	vgaxo(Crtx, 0x5A, l & 0xFF);
...
s3ident(void)
{
	...
	if(crt30 == 0xE1){					/* Trio64 */
		...
		return 1;
	if((crt30&0xF0) == 0xC0 || (crt30&0xF0) == 0xD0)
		return 1;
	return 0;
}
...
/*
	...
	if(pcidevno == -1){
		pcicfg.vid = 0x1002;
		pcicfg.did = 0x4354;
		if((pcidevno = pcimatch(0, 0, &pcicfg)) == -1)
			return 0;
		pcidevno--;
	}
		l = PADDR(aperture);
		pcicfgw(0, pcidevno, 0, 0x10, &l, 4);
*/
