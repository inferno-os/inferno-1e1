...
setvgadev(char* ctlr, Mode* mode)
{
	...
	vgac = 0;
	for(v = vgactlr; v; v = v->link){
		if(ctlr){
			if(v->name && strcmp(ctlr, v->name) == 0){
				vgac = v;
				break;
			}
		}
		else if(v->ident && (*v->ident)()){
			vgac = v;
			break;
		}
	}
...
setscreen(Mode* mode)
{
	...
	if(findvga() == 0)
		return;
	/* remap to synchronise addresses with pci.c */
	base = (ulong)pcimemmap(0, vgaslot, 0, 0, 0);
	if(base == 0)
		return;
	/* the Be ROM leaves the screen set for 640x480 mode */
	mode->aperture = (uchar*)base;
...
findvga(void)
{
	...
	vgaslot = -1;
	for(i = 1; i < MaxPCI; i++) {
		memset(&pcicfg, 0, sizeof(pcicfg));
		pcicfgr(0, i, 0, 0, &pcicfg, sizeof(pcicfg));
		if(pcicfg.vid != 0xFFFF &&
		   (pcicfg.baseclass == 0 && pcicfg.subclass == 1 || pcicfg.baseclass == 3)){
			base = pcicfg.baseaddr[0];
			if(base != 0){
				vgaslot = i;
				break;
			}
	...
	if(vgaslot < 0)
		return 0;
	return (ulong*)(KSEG2|base);
}

ulong
getspace(ulong size, ulong align)
{
	if(vgaslot < 0)
		return 0;
	USED(size);
	USED(align);
	return (ulong)pcimemmap(0, vgaslot, 0, 0, 0);
}
