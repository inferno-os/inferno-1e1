...
tcpclose(Conv *c)
{
	...
	qclose(c->rq);	/* Not interested in data anymore */
	qclose(c->wq);
	qclose(c->eq);
...
*/
		...
		if(ssize != 0){
			...
			if(x <= 0)
				x = 1;
			if(x <= 200/MSPTICK)
				x = 200/MSPTICK + 1;	/* wait for remote delayed ack (forsyth) */
			tcb->timer.start = x;
			if(tcb->timer.state != TimerON)
				tcpgo(&tcb->timer);
...
tcptimeout(void *arg)
{
	...
	default:
		...
		netlog(2, "rexmit(%d,%d,%d,%d,%d)\n", tcb->timer.start, tcb->srtt, tcb->mdev, tcb->backoff, tcb->rtt_timer.start);
		tcprxmit(s);
		break;
	case Finwait2:
		if(--(tcb->f2counter) <= 0)
