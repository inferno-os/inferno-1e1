...
struct Ilhdr
{
	byte	sum[2];	/* Checksum including header */
	byte	len[2];	/* Packet length */
	byte	type;		/* Packet type */
	byte	spec;		/* Special */
	byte	src[2];	/* Src port */
	byte	dst[2];	/* Dst port */
	byte	id[4];	/* Sequence id */
	byte	ack[4];	/* Acked sequence */
};
...
Block*
tcpuncompress(Tcpc *comp, Block *b, ushort type)
{
	byte	*cp, changes;
	short	i;
	int	iplen, len;
	...
	if(type == Pvjutcp) {
		...
		comp->err = 0;
netlog(Logcompress, "uncompressed %d\n", comp->lastrecv);
		/*
