...
#define Nmpegtab (sizeof(mpegtab)/sizeof(Dirtab))
...
mpegreset(void)
{
	...
	setvec(Int0vec+mpegconf.irq, mpegintr, 0);
}
...
{
	return devwalk(c, name, mpegtab, Nmpegtab, devgen);
}

void
mpegstat(Chan *c, char *db)
{
	devstat(c, db, mpegtab, Nmpegtab, devgen);
}
...
mpegopen(Chan *c, int omode)
{
	...
	return devopen(c, omode, mpegtab, Nmpegtab, devgen);
}

void
mpegcreate(Chan *c, char *name, int omode, ulong perm)
{
...
mpegread(Chan *c, void *a, long n, ulong off)
{
	...
	default:
		error(Eperm);
	case Qdir:
		return devdirread(c, a, n, mpegtab, Nmpegtab, devgen);
	case Qctl:
...
Block*
mpegbread(Chan *c, long n, ulong offset)
{
...
long
mpegwrite(Chan *c, char *a, long n, ulong)
{
	Audio *t;
	int i, nf, l, x;
...
long
mpegbwrite(Chan *c, Block *bp, ulong offset)
{
