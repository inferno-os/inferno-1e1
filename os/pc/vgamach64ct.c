...
mach64ctinit(Mode* mode)
{
	 ...
	 */
	if(aperture == 0){
		if((aperture = getspace(8*1024*1024, 16*1024*1024)) == 0)
			return;
	
		l = PADDR(aperture);
		pcicfgw(0, pcidevno, 0, 0x10, &l, 4);
	
		l = ((l/(4*1024*1024))<<4)|0x02;
		iow32(ConfigCntl, l);
	}

	mode->aperture = (uchar*)aperture;
...
load(Cursor *c)
{
	...
	mem = ((uchar*)gscreen.base) + storage;
