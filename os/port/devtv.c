...
#define Ntvtab (sizeof(tvtab)/sizeof(Dirtab))
...
int vsync, hsync;
ulong xtalfreq;
ushort cropleft, cropright;
ushort cropbottom, croptop;
Rectangle window, capwindow;

void setreg(int, int);
void setbt812reg(int, int);
void videoinit(void);
void createwindow(Rectangle);
void setcontrols(int, uchar);
void setcolorkey(int, int, int, int, int, int);
void soundinit(void);
void setvolume(int, int);
void setbass(int);
void settreble(int);
void setsoundsource(int);
void tunerinit(void);
void settuner(int, int);
void setvideosource(int);
int waitvideosignal(void);
void freeze(int);
void setsvideo(int);
void setinputformat(int);
...
Chan *
tvclone(Chan *c, Chan *nc)
{
	return devclone(c, nc);
}

int
tvwalk(Chan *c, char *name)
{
	return devwalk(c, name, tvtab, Ntvtab, devgen);
}

void
tvstat(Chan *c, char *db)
{
	devstat(c, db, tvtab, Ntvtab, devgen);
}

Chan *
tvopen(Chan *c, int omode)
{
	return devopen(c, omode, tvtab, Ntvtab, devgen);
}

void
tvcreate(Chan *c, char *name, int omode, ulong perm)
{
...
long
tvread(Chan *c, void *a, long n, ulong offset)
{
	static void *frame;
	static int size;
	...
	case Qdir:
		return devdirread(c, a, n, tvtab, Ntvtab, devgen);
	case Qdata:
...
Block*
tvbread(Chan *c, long n, ulong offset)
{
	return devbread(c, n, offset);
}

long
tvwrite(Chan *c, char *a, long n, ulong offset)
{
...
long
tvbwrite(Chan *c, Block *bp, ulong offset)
{
...
settuner(int channel, int finetune)
{
	...
	if (channel < 0 || channel > sizeof(hrcfreq)/sizeof(long))
		error(Ebadarg);
...
ushort actpixs = 720;
ulong Hdesired = 13500000L;

/*			   NTSC-M  NTSC-443  EXTERNAL */
ushort horzfreq[] =	{   15734,    15625,        0 };
ushort Vdelay[]	=	{      22,       25,       25 };
ushort s2b[] =		{      90,       90,        0 };
ushort actlines[] =	{     485,      485,      575 };
ulong subcarfreq[] =	{ 3579545,  4433619,  4433619 };
...
ushort panmask[] = { 0xFFFE, 0xFFFC, 0xFFFF, 0xFFFF, 0xFFFE };
...
volatile ushort *fb = (ushort *)EISA(MemAddr);
uchar yuvpadbound[] = { 4, 12, 4, 2, 6 }; 
