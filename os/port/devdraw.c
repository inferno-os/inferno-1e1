...
#define Ndrawtab (sizeof(drawtab)/sizeof(Dirtab))
...
drawgen(Chan *c, Dirtab *tab, int x, int s, Dir *dp)
{
	...
	if(c->qid.path == CHDIR){
		/*
		 * Index 0 of top directory is the "new" directory,
		 * [1..n] are clients [0..n).
		 */
		if(s == 0){
			qid = (Qid){CHDIR|Qnew, 0};
			devdir(c, qid, "new", 0, eve, CHDIR|0555, dp);
			return 1;
		}
		--s;
		if(s >= sdraw.nclient)
	...
	devdir(c, qid, tab->name, 0, eve, tab->perm, dp);
	return 1;
}
...
dstflush(int dstid, Memimage *dst, Rectangle r)
{
	...
	if(dst->layer == nil)
		return;
	if(dst->layer->screen->image->base != screenimage.base)
		return;
...
drawchar(Memimage *dst, Point p, Memimage *src, Point *sp, DImage *font, int index)
{
	...
	memdraw(dst, r, src, *sp, font->image, Pt(fc->minx, fc->miny));
	p.x += fc->width;
	sp->x += fc->width;
	return p;
}
...
drawattach(char *spec)
{
	...
	if(screenimage.base == nil){
		screenimage.base = attachscreen(&screenimage.r, &screenimage.ldepth, &width);
		screenimage.width = width;
		screenimage.clipr  = screenimage.r;
		if(screenimage.base == 0)
...
drawwalk(Chan *c, char *name)
{
	...
	if(c->qid.path==CHDIR && strcmp(name, "new")==0){
		cl = drawnewclient();
		if(cl == 0)
			return 0;
...
void
drawcreate(Chan *c, char *n, int o, ulong m)
{
	USED(c);
	USED(n);
	USED(o);
...
Block*
drawbread(Chan *c, long n, ulong offset)
{
...
long
drawbwrite(Chan *c, Block *bp, ulong offset)
{
	...
	while((n-=m) > 0){
		...
		case 'P':
			...
			if(pp == nil)
				error(Enomem);
			doflush = 0;
			if(dstid==0 || (dst->layer && dst->layer->screen->image->base == screenimage.base))
		...
		case 't':
			...
			if(a[1])
				...
				memltorearn(lp, nw);
			if(lp[0]->layer->screen->image->base == screenimage.base)
				for(j=0; j<nw; j++)
