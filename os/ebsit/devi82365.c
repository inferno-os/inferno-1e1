...
i82365reset(void)
{
	...
	for(i = 0; i < ncontroller; i++){
		...
		for(j = 0; j < cp->nslot; j++){
			...
			wrreg(pp, Rcscic, ((CSRR_PCMCIA_IRQ1)<<4) | Fchangeena);
			rdreg(pp, Rcsc);
		}
	}
	...
	/* for card management interrupts */
 	if(ncontroller)
		intrenable(1, CSRR_PCMCIA_IRQ1, i82365intr, 0);
	print("i82365 reset\n");
}
...
Block*
i82365bread(Chan *c, long n, ulong o)
{
	return devbread(c, n, o);

}
Chan*
i82365clone(Chan *oc, Chan *nc)
{
	return devclone(oc, nc);
}
void
...
long
i82365bwrite(Chan *c, Block *b, ulong o)
{
	return devbwrite(c, b, o);
}
