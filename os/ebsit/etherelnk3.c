 ...
 *
 *	9058 PCMCIA	3C589[B]-[TP|COMBO] ( and works with C too)
 *
 *	627C MCA	3C529
 *	627D MCA	3C529-TP
 */
...
void outsl(int ioaddr, void *data, int count) {
   	int i;
	ulong *p = (ulong *)data;
...
void outsw(int ioaddr, void *data, int count) {
    	int i;
...
void insl(int ioaddr, void *buffer, int count) {
    int i;
	...
	*p = inl(ioaddr);
    ...
    if ((ulong) p > (ulong) buffer + (count*4))
	print("insl overflowed\n");
   
}

/* Multi-word input from an I/O address */
void insw(int ioaddr, void *buffer, int count) {
...
interrupt(Ureg*, void* arg)
{
	...
	MESSAGE;	/* Turn on MESSAGE during ether */
	if (arg == 0) {
		print("No ether arg passed..spining\n");
		while(1);
		}
	ether = arg;
	port = ether->port;
	...
	while((status = STATUS(port)) & (interruptMask|interruptLatch)){
		...
		if(status & (transferInt|rxComplete)){
			...
			COMMAND(port, AcknowledgeInterrupt, transferInt|rxComplete);
		}
		...
		if(status & updateStats){
			...
			status &= ~updateStats;
			USED(status);
			COMMAND(port, AcknowledgeInterrupt, updateStats);
		 ...
		 */
		if(status & rxEarly){
			print("-shouldn't happen, rxEarly-");
		...
		if(status & interruptMask)
			{
			print("panic..uncovered interrupt\n");
	...
	MESSAGE;			/* turn off message after ether */
...
etherelnk3reset(Ether* ether)
{
	...
	print("Receive buffer allocated at %8.8ux\n",ctlr->rbp);
	/*
