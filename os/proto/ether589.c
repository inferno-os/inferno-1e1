...
receive(Ether *ether)
{
	...
	while(((status = ins(port+RxStatus)) & RxEmpty) == 0){
		...
		if(status & RxError){
			...
			len = (len + 3) & ~3;
			insssw(port+Fifo, &ether->rpkt, len>>1);
...
write509(Ether *ether, void *buf, long n)
{
	...
	outs(port+Fifo, 0);
	outsssw(port+Fifo, buf, Eaddrlen/2);
	outsssw(port+Fifo, ether->ea, Eaddrlen/2);
	outsssw(port+Fifo, (uchar*)buf+2*Eaddrlen, (len-2*Eaddrlen)/2);
	ether->outpackets++;
...
reset(Ether *ether)
{
	...
	outs(port + AddressConfig, x & 0xf0);
	...
	settrap(Tintr2, interrupt, ether);
	return 0;
}
