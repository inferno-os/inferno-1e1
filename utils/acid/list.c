...
listlocals(Map *map, Symbol *fn, ulong fp)
{
	...
	for(i = 0; localsym(&s, i); i++) {
		if(s.s0.sclass != CAUTO)
			continue;
		if(s.s0.sname[0] == '.')
			continue;

		if(get4(map, fp-s.s0.svalue, &val) > 0) {
			*tail = listvar(s.s0.sname, val);
			tail = &(*tail)->next;
		}
	}
	return l2;
...
listparams(Map *map, Symbol *fn, ulong fp)
{
	...
	s = *fn;
	for(i = 0; localsym(&s, i); i++) {
		if (s.s0.sclass != CPARAM)
			continue;

		if(get4(map, fp+s.s0.svalue, &v) > 0) {
			*tail = listvar(s.s0.sname, v);
			tail = &(*tail)->next;
		}
	}
	return l2;
...
trlist(Map *map, ulong pc, ulong sp, Symbol *sym)
{
	...
	*tail = q;
	...
	q->lstore.u0.sl = l;
	l->lstore.u0.sival = sym->s0.svalue;
	l->lstore.fmt = 'X';
