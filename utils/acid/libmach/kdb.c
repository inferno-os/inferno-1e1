...
plocal(Instr *i)
{
	...
	if (!findsym(i->addr, CTEXT, &s) || !findlocal(&s, FRAMENAME, &s))
		return -1;
	if (s.s0.svalue > i->simm13) {
		if(getauto(&s, s.s0.svalue-i->simm13, CAUTO, &s)) {
			bprint(i, "%s+%d(SP)", s.s0.sname, s.s0.svalue);
			return 1;
		}
	} else {
		offset = i->simm13-s.s0.svalue;
		if (getauto(&s, offset-4, CPARAM, &s)) {
			bprint(i, "%s+%d(FP)", s.s0.sname, offset);
			return 1;
		}
	}
	return -1;
...
address(Instr *i)
{
	...
	if(i->rs1 == 2	&& findsym(off, CANY, &s)
			&& s.s0.svalue-off < 4096
			&& (s.s0.sclass == CDATA || s.s0.sclass == CTEXT)) {
		if(off==s.s0.svalue && s.s0.sname[0]=='$'){
			off1 = 0;
			get4(mymap, s.s0.svalue, &off1);
			if(off1 && findsym(off1, CANY, &s2) && s2.s0.svalue == off1){
				bprint(i, "$%s(SB)", s2.s0.sname);
				return;
			}
		}
		bprint(i, "%s", s.s0.sname);
		if (s.s0.svalue != off)
			bprint(i, "+%lux", s.s0.svalue-off);
		bprint(i, "(SB)");
		return;
	}
