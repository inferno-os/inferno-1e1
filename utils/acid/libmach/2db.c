...
struct	operand
{
	...
	short	ext;
	union {
		long	oimmediate;	/* sign-extended integer byte/word/long */
		struct	{		/* index mode displacements */
			long	odisp;
			long	oouter;
		} s0;
		char	ofloater[24];	/* floating point immediates */
	} u0;
};
...
getimm(Inst *ip, Operand *ap, int mode)
{
	...
	case EAM_B:	/* byte */
	case EAALL_B:
		return i8(ip, &ap->u0.oimmediate);
	case EADI_W:	/* word */
	case EAALL_W:
		return i16(ip, &ap->u0.oimmediate);
	case EADI_L:	/* long */
	case EAALL_L:
		return i32(ip, &ap->u0.oimmediate);
	case EAFLT:	/* floating point - size in bits 10-12 or word 1 */
		...
		case 1:	/* single precision real */
			ap->eatype = IREAL;
			return getshorts(ip, ap->u0.ofloater, 2);
		case 2:	/* extended precision real - not supported */
			ap->eatype = IEXT;
			return getshorts(ip, ap->u0.ofloater, 6);
		case 3: /* packed decimal real - not supported */
			ap->eatype = IPACK;
			return getshorts(ip, ap->u0.ofloater, 12);
		case 4:	/* integer word */
			return i16(ip, &ap->u0.oimmediate);
		case 5:	/* double precision real */
			ap->eatype = IDBL;
			return getshorts(ip, ap->u0.ofloater, 4);
		case 6:	/* integer byte */
			return i8(ip, &ap->u0.oimmediate);
		default:
...
getdisp(Inst *ip, Operand *ap)
{
	...
	if ((ext&0x100) == 0) {		/* indexed with 7-bit displacement */
		ap->u0.s0.odisp = ext&0x7f;
		if (ap->u0.s0.odisp&0x40)
			ap->u0.s0.odisp |= ~0x7f;
		return 1;
	}
	...
	case 0x10:
		break;
	case 0x20:
		if (i16(ip, &ap->u0.s0.odisp) < 0)
			return -1;
		break;
	case 0x30:
		if (i32(ip, &ap->u0.s0.odisp) < 0)
			return -1;
		break;
	default:
	...
	case 0x02:		/* 16 bit displacement */
		return i16(ip, &ap->u0.s0.oouter);
	case 0x03:		/* 32 bit displacement */
		return i32(ip, &ap->u0.s0.oouter);
	default:
		break;
	}
	return 1;
...
ea(Inst *ip, int ea, Operand *ap, int mode)
{
	...
	case 0x07:
		...
		case 0x00:
			type = Abs;
			ap->eatype = ABS;
			if (i16(ip, &ap->u0.oimmediate) < 0)
				return -1;
			break;
		case 0x01:
			type = Abs;
			ap->eatype = ABS;
			if (i32(ip, &ap->u0.oimmediate) < 0)
				return -1;
			break;
		case 0x02:
			type = PCrel;
...
decode(Inst *ip, Optable *op)
{
	...
	for (i = 0; i < nelem(op->opdata) && op->opdata[i]; i++) {
		...
		case OP8:	/* weird movq instruction */
			ap->eatype = IMM;
			ap->u0.oimmediate = opcode&0xff;
			if (opcode&0x80)
				ap->u0.oimmediate |= ~0xff;
			break;
		case I8:	/* must be two-word opcode */
			ap->eatype = IMM;
			ap->u0.oimmediate = ip->raw[1]&0xff;
			if (ap->u0.oimmediate&0x80)
				ap->u0.oimmediate |= ~0xff;
			break;
		...
		case BR8:	/* branch displacement format */
			ap->eatype = IMM;
			ap->u0.oimmediate = opcode&0xff;
			if (ap->u0.oimmediate == 0) {
				if (i16(ip, &ap->u0.oimmediate) < 0)
					return -1;
			} else if (ap->u0.oimmediate == 0xff) {
				if (i32(ip, &ap->u0.oimmediate) < 0)
					return -1;
			} else if (ap->u0.oimmediate & 0x80)
				ap->u0.oimmediate |= ~0xff;
			break;
...
plocal(Inst *ip, Operand *ap)
{
	...
	if (ret)
		bprint(ip, "%s+%lux", s.s0.sname, offset);
	else
none:		bprint(ip, "%lux", ap->u0.s0.odisp);
}
...
prindex(Inst *ip, int reg, Operand *ap)
{
	...
	case 0x12:
	case 0x13:
		ip->curr += symoff(ip->curr, left, ap->u0.s0.oouter, CANY);
		if (pidx(ip, ext, reg, "((%s)", "((%s)", 0))
			bprint(ip, ")");
		break;
	case 0x15:
...
pea(int reg, Inst *ip, Operand *ap)
{
	...
	case APdec:
		...
		break;
	case PDisp:
		ip->curr += symoff(ip->curr, left, ip->addr+2+ap->u0.s0.odisp, CANY);
		break;
	case PXD:
		prindex(ip, 16, ap);
		break;
	case ADisp:	/* references off the static base */
		if (reg == 6 && mach->sb && ap->u0.s0.odisp) {
			ip->curr += symoff(ip->curr, left, ap->u0.s0.odisp+mach->sb, CANY);
			bprint(ip, "(SB)", reg);
			break;
		}
		...
		if (reg == 7)
			...
			ip->curr += symoff(ip->curr, left, ap->u0.s0.odisp, CANY);
		bprint(ip, "(A%d)", reg);
		break;
	case BXD:
		prindex(ip, reg+8, ap);
		break;
	case ABS:
		ip->curr += symoff(ip->curr, left, ap->u0.oimmediate, CANY);
		bprint(ip, "($0)");
		break;
	case IMM:
		*ip->curr++ = '$';
		ip->curr += symoff(ip->curr, left-1, ap->u0.oimmediate, CANY);
		break;
	case IREAL:
		*ip->curr++ = '$';
		ip->curr += beieeesftos(ip->curr, left-1, (void*) ap->u0.ofloater);
		break;
	case IDBL:
		*ip->curr++ = '$';
		ip->curr += beieeedftos(ip->curr, left-1, (void*) ap->u0.ofloater);
		break;
	case IPACK:
		bprint(ip, "$#");
		for (i = 0; i < 24 && ip->curr < ip->end-1; i++) {
			_hexify(ip->curr, ap->u0.ofloater[i], 1);
			ip->curr += 2;
		}
		break;
	case IEXT:
		bprint(ip, "$#");
		ip->curr += beieee80ftos(ip->curr, left-2, (void*)ap->u0.ofloater);
		break;
	default:
	...
	for (; *fmt && ip->curr < ip->end; fmt++) {
		...
		case 'i':	/* immediate operand */
			ip->curr += symoff(ip->curr, ip->end-ip->curr,
					ip->and[currand++].u0.oimmediate, CANY);
			break;
		...
		case 't':	/* text offset */
			ip->curr += symoff(ip->curr, ip->end-ip->curr,
				ip->and[currand++].u0.oimmediate+ip->addr+2, CTEXT);
			break;
	...
	for (i = 0; i < nelem(op->opdata) && op->opdata[i]; i++) {
		...
		case BREAC:	/* EAC JMP or CALL operand */
			...
			if ((opcode&0x038) == 0x28 || (opcode&0x3f) == 0x3a) {
				if (i16(ip, &ip->and[i].u0.oimmediate) < 0)
					return -1;
			} else {
				t = easize(ip, opcode&0x3f, mode);
				if (t < 0)
		...
		case BR32:	/* 32 bit branch displacement */
			if (i32(ip, &ip->and[i].u0.oimmediate) < 0)
				return -1;
			break;
		case I32:	/* 32 bit immediate */
			ip->n += 2;
		...
		case BR8:	/* loony branch displacement format */
			...
			if (t == 0) {
				...
				ip->and[i].u0.oimmediate = t;
				if (t & 0x80)
					ip->and[i].u0.oimmediate |= ~0xff;
			}
			break;
...
eaval(Inst *ip, Operand *ap, Rgetter rget)
{
	...
	case PDisp:
		return ip->addr+2+ap->u0.s0.odisp;
	case ADisp:
		sprint(buf, "A%d", reg);
		return ap->u0.s0.odisp+(*rget)(mymap, buf);
	case ABS:
		return ap->u0.oimmediate;
	default:
		return 0;
	}
}
...
m68020foll(Map *map, ulong pc, Rgetter rget, ulong *foll)
{
	...
	for (j = 0; j < nelem(op->opdata) && op->opdata[j]; j++) {
		...
		case BR32:	/* FBcc */
			foll[0] = pc+i.n*2;
			foll[1] = pc+2+i.and[j].u0.oimmediate;
			return 2;
