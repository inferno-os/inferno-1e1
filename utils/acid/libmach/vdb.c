...
plocal(Instr *i, char *m, char r, int store)
{
	...
	if (!findsym(i->addr, CTEXT, &s) || !findlocal(&s, FRAMENAME, &s))
		return 0;
	if (s.s0.svalue > i->immediate) {
		if(!getauto(&s, s.s0.svalue-i->immediate, CAUTO, &s))
			return 0;
		reg = "(SP)";
		offset = i->immediate;
	} else {
		offset = i->immediate-s.s0.svalue;
		if (!getauto(&s, offset-4, CPARAM, &s))
			return 0;
		reg = "(FP)";
	}
	if (store)
		bprint(i, "%s\t%c%d,%s+%d%s", m, r, i->rt, s.s0.sname, offset, reg);
	else
		bprint(i, "%s\t%s+%d%s,%c%d", m, s.s0.sname, offset, reg, r, i->rt);
	return 1;
}
