...
i386trace(Map *map, ulong pc, ulong sp, ulong link, Tracer trace)
{
	...
	while(findsym(pc, CTEXT, &s)) {
		...
		if(pc != s.s0.svalue) {	/* not at first instruction */
			if(findlocal(&s, FRAMENAME, &f) == 0)
				break;
			sp += f.s0.svalue-mach->szaddr;
		}
...
i386frame(Map *map, ulong addr, ulong pc, ulong sp, ulong link)
{
	...
	while (findsym(pc, CTEXT, &s)) {
		...
		if(pc != s.s0.svalue) {	/* not first instruction */
			if(findlocal(&s, FRAMENAME, &f) == 0)
				break;
			sp += f.s0.svalue-mach->szaddr;
		}
...
plocal(Instr *ip)
{
	...
	if (s.s0.svalue > ip->disp) {
		ret = getauto(&s, s.s0.svalue-ip->disp-mach->szaddr, CAUTO, &s);
		reg = "(SP)";
	} else {
		offset -= s.s0.svalue;
...
immediate(Instr *ip, long val)
{
	...
	if (findsym(val, CANY, &s)) {
		...
		if (w < 4096) {
			if (w)
				bprint(ip, "%s+%lux(SB)", s.s0.sname, w);
			else
				bprint(ip, "%s(SB)", s.s0.sname);
			return;
		}
	}
	bprint(ip, "%lux", val);
