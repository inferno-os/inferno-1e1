...
localaddr(Map *map, char *fn, char *var, long *r, Rgetter rget)
{
	...
	if(mach->link)
		...
		link = 0;
	fp = machdata->findframe(map, s.s0.svalue, pc, sp, link);
	if (fp == 0) {
	...
	case CPARAM:		/* assume address size is stack width */
		*r = fp + s.s0.svalue + mach->szaddr;
		break;
	default:
		werrstr("local variable not found: %d", s.s0.sclass);
		return -1;
	}
	return 1;
}
...
symoff(char *buf, int n, long v, int space)
{
	...
	if (s.s0.stype != 't' && s.s0.stype != 'T' && delta >= 4096)
		return snprint(buf, n, "%lux", v);
	else if (delta)
		return snprint(buf, n, "%s+%lux", s.s0.sname, delta);
	else
		return snprint(buf, n, "%s", s.s0.sname);
}
/*
...
cisctrace(Map *map, ulong pc, ulong sp, ulong link, Tracer trace)
{
	...
	while(pc && opc != pc) {
		...
		if (!found)
			break;
		if(strcmp(STARTSYM, s.s0.sname) == 0 || strcmp(PROFSYM, s.s0.sname) == 0)
			break;
...
risctrace(Map *map, ulong pc, ulong sp, ulong link, Tracer trace)
{
	...
	while(findsym(pc, CTEXT, &s)) {
		...
		if(pc == s.s0.svalue)	/* at first instruction */
			f.s0.svalue = 0;
		else if(findlocal(&s, FRAMENAME, &f) == 0)
			break;

		oldpc = pc;
		if(s.s0.stype == 'L' || s.s0.stype == 'l' || pc <= s.s0.svalue+mach->pcquant)
			pc = link;
		else
			if (get4(map, sp, (long *) &pc) < 0)
				break;

		if(pc == 0 || (pc == oldpc && f.s0.svalue == 0))
			break;
...
riscframe(Map *map, ulong addr, ulong pc, ulong sp, ulong link)
{
	...
	while (findsym(pc, CTEXT, &s)) {
		...
		if (s.s0.stype == 'L' || s.s0.stype == 'l' || pc-s.s0.svalue <= mach->szaddr*2)
			pc = link;
		else
		if (get4(map, sp-f.s0.svalue, (long *)&pc) < 0)
			break;
	}
	return 0;
}
