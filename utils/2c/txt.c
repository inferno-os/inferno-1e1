...
naddr(Node *n, Adr *a, int x)
{
	...
	case OADDR:
	case OIND:
		naddr(n->u0.s0.nleft, a, x);
		goto noadd;

	case OREGISTER:
		a->sym = S;
		a->type = n->u0.s2.nreg;
		a->u0.s0.offset = n->u0.s2.noffset;
		a->u0.s0.displace = 0;
		break;
	...
	case OCONST:
		...
		if(typefd[n->type->etype]) {
			a->type = D_FCONST;
			a->u0.dval = n->u0.nfconst;
			break;
		}
		a->type = D_CONST;
		a->u0.s0.offset = n->u0.nvconst;
		break;
...
gopcode(int o, Type *ty, int gf, Node *f, int gt, Node *t)
{
	...
	if(gf == D_TREE) {
		if(f->op == OINDEX) {
			...
			cgen(f->u0.s0.nright, fidx, f->u0.s0.nright);
		}
	}
	tidx = D_NONE;
	if(gt == D_TREE) {
		if(t->op == OINDEX) {
			...
			cgen(t->u0.s0.nright, tidx, t->u0.s0.nright);
			if(gf == D_TOS)
				adjsp(v - argoff);
		}
	}
	...
	if(gf == D_TREE) {
		...
		p->from.type = gf;
		if(gf == D_CONST) {
			p->from.u0.s0.offset = (long)f;
			if(typefd[i]) {
				p->from.type = D_FCONST;
				p->from.u0.dval = (long)f;
			}
		}
	}
	p->as = opxt[o][i];
