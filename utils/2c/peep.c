...
compat(Adr *a, Adr *b)
{
	...
	if(o >= D_A0 && o < D_A0+NREG) {
		if(o != (b->type&D_MASK))
			return 0;
		if(a->u0.s0.offset != b->u0.s0.offset)
			return 0;
		o = a->type & I_MASK;
		if(o == I_INDIR) {
			o = b->type & I_MASK;
...
aregind(Adr *a)
{
	...
	while(a->u0.s0.offset == 0 && a->index == D_NONE)
		return t & D_MASK;
	return D_NONE;
}
...
copyas(Adr *a, Adr *v)
{
	...
	if(v->type == D_AUTO || v->type == D_PARAM) {
		if(v->u0.s0.offset == a->u0.s0.offset)
			return 1;
		return 0;
	}
	return 0;
...
tasas(Adr *a, Adr *v)
{
	...
	if(v->type != t)
		return 0;
	if(a->u0.s0.displace != v->u0.s0.displace)
		return 0;
	return 1;
}
