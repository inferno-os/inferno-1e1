/* 10/15/96 kevin:  resolved structure names to what they should be */
...
cgen(Node *n, int result, Node *nn)
{
	...
	case ODOT:
		...
		if(result != D_NONE) {
			...
			nod.u0.s2.noffset += r->u0.nvconst;
			nod.type = n->type;
			cgen(&nod, result, nn);
		}
		break;
	...
	case OCOND:
		...
		inargs++;
		doinc(r->u0.s0.nleft, PRE);
		cgen(r->u0.s0.nleft, result, nn);
		doinc(r->u0.s0.nleft, POST);
		gbranch(OGOTO);
		patch(p1, pc);
		p1 = p;

		doinc(r->u0.s0.nright, PRE);
		cgen(r->u0.s0.nright, result, nn);
		doinc(r->u0.s0.nright, POST);
		patch(p1, pc);
		inargs--;
		break;
	...
	case OIND:
		...
		if(l->op == OADD) {
			if(l->u0.s0.nleft->op == OCONST) {
				nod.u0.s2.noffset += l->u0.s0.nleft->u0.nvconst;
				l = l->u0.s0.nright;
			} else
			if(l->u0.s0.nright->op == OCONST) {
				nod.u0.s2.noffset += l->u0.s0.nright->u0.nvconst;
				l = l->u0.s0.nleft;
			}
		}
		cgen(l, lg, l);
	...
	case OAND:
		...
		if(l->op == OCAST) {
			if(typec[l->u0.s0.nleft->type->etype])
			if(!(r->u0.nvconst & ~0xff)) {
				l = l->u0.s0.nleft;
				goto binop;
			}
			if(typeh[l->u0.s0.nleft->type->etype])
			if(!(r->u0.nvconst & ~0xffff)) {
				l = l->u0.s0.nleft;
				goto binop;
			}
		}
...
lcgen(Node *n, int result, Node *nn)
{
	...
	switch(n->op) {
	case OCOMMA:
		cgen(n->u0.s0.nleft, D_NONE, n->u0.s0.nleft);
		doinc(n->u0.s0.nleft, POST);
		doinc(n->u0.s0.nright, PRE);
		lcgen(n->u0.s0.nright, result, nn);
		break;

	case OCOND:
		doinc(n->u0.s0.nleft, PRE);
		boolgen(n->u0.s0.nleft, 1, D_NONE, Z, n->u0.s0.nleft);
		p1 = p;

		inargs++;
		doinc(n->u0.s0.nright->u0.s0.nleft, PRE);
		lcgen(n->u0.s0.nright->u0.s0.nleft, result, nn);
		doinc(n->u0.s0.nright->u0.s0.nleft, POST);
		gbranch(OGOTO);
		patch(p1, pc);
		p1 = p;

		doinc(n->u0.s0.nright->u0.s0.nright, PRE);
		lcgen(n->u0.s0.nright->u0.s0.nright, result, nn);
		doinc(n->u0.s0.nright->u0.s0.nright, POST);
		patch(p1, pc);
		inargs--;
		break;
...
boolgen(Node *n, int true, int result, Node *nn, Node *post)
{
	...
	case OCOND:
		...
		inargs++;
		doinc(r->u0.s0.nleft, PRE);
		boolgen(r->u0.s0.nleft, true, result, nn, r->u0.s0.nleft);
		if(result != D_NONE) {
			doinc(r->u0.s0.nleft, POST);
			gbranch(OGOTO);
			patch(p1, pc);
			...
			doinc(r->u0.s0.nright, PRE);
			boolgen(r->u0.s0.nright, !true, result, nn, r->u0.s0.nright);
			doinc(r->u0.s0.nright, POST);
			patch(p1, pc);
			inargs--;
			break;
...
sugen(Node *n, int result, Node *nn, long w)
{
	...
	case OCONST:
		if(n->type && typev[n->type->etype]) {
			...
			gopcode(OAS, types[TLONG], D_CONST, nodconst((long)(n->u0.nvconst>>32)),
				lg|I_INDINC, n);
			gopcode(OAS, types[TLONG], D_CONST, nodconst((long)(n->u0.nvconst)),
				lg|I_INDINC, n);
			regfree(lg);
			break;
		}
	...
	case OAS:
		if(result == D_NONE) {
			sugen(n->u0.s0.nright, D_TREE, n->u0.s0.nleft, w);
			break;
		}
		sugen(n->u0.s0.nright, D_TREE, nodrat, w);	/* could do better */
		warn(n, "non-interruptable temporary");
		sugen(nodrat, D_TREE, n->u0.s0.nleft, w);
	...
	case OFUNC:
		...
		if(result == D_TOS) {
			adjsp(s - argoff + w);
			v = argoff;
			gargs(n->u0.s0.nright);
			gopcode(OADDR, types[TSHORT], D_NONE, nn, result, nn);
			p->to.type = D_STACK;
			p->to.u0.s0.offset = argoff - v;
		} else
		if(result == D_TREE) {
			v = argoff;
			gargs(n->u0.s0.nright);
	...
	case OCOND:
		...
		inargs++;
		doinc(n->u0.s0.nright->u0.s0.nleft, PRE);
		sugen(n->u0.s0.nright->u0.s0.nleft, result, nn, w);
		doinc(n->u0.s0.nright->u0.s0.nleft, POST);
		gbranch(OGOTO);
		patch(p1, pc);
		p1 = p;
		...
		doinc(n->u0.s0.nright->u0.s0.nright, PRE);
		sugen(n->u0.s0.nright->u0.s0.nright, result, nn, w);
		doinc(n->u0.s0.nright->u0.s0.nright, POST);
		patch(p1, pc);
		inargs--;
		break;
	...
	case OCOMMA:
		...
		doinc(n->u0.s0.nleft, POST);
		doinc(n->u0.s0.nright, PRE);
		sugen(n->u0.s0.nright, result, nn, w);
		break;
	}
	return;
