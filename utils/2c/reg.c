...
loop11:
	...
	for(r = firstr; r != R; r = r1) {
		r1 = r->link;
		if(r1 && r1->u0.active && !r->u0.active) {
			prop(r, zbits, zbits);
			i = 1;
		}
	}
...
addreg(Adr *a, int rn)
{
	...
	if(x == (I_INDEX3|D_NONE)) {
		a->type = rn | I_INDIR;
		a->index = D_NONE;
		a->u0.s0.offset = a->u0.s0.displace;
		a->u0.s0.displace = 0;
		return;
	}
	if(x != D_NONE) {
