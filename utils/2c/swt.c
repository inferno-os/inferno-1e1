...
direct:
	...
	p->from.u0.s0.offset = nstatic - v*2;
...
outstring(char *s, long n)
{
	...
	while(n) {
		...
		if(mnstring >= NSNAME) {
			...
			memmove(p->to.u0.sval, string, NSNAME);
			p->from.u0.s0.offset = nstring - NSNAME;
			p->from.u0.s0.displace = NSNAME;
			mnstring = 0;
		}
		n--;
	}
...
sextern(Sym *s, Node *a, long o, long w)
{
	...
	for(e=0; e<w; e+=NSNAME) {
		...
		p->from.u0.s0.offset += o+e;
		p->from.u0.s0.displace = lw;
		memmove(p->to.u0.sval, a->u0.ncstring+e, lw);
	}
}
...
gextern(Sym *s, Node *a, long o, long w)
{
	if(a->op == OCONST && typev[a->type->etype]) {
		gpseudo(ADATA, s, D_CONST, (long)(a->u0.nvconst>>32));
		p->from.u0.s0.offset += o;
		p->from.u0.s0.displace = 4;
		gpseudo(ADATA, s, D_CONST, (long)(a->u0.nvconst));
		p->from.u0.s0.offset += o + 4;
		p->from.u0.s0.displace = 4;
		return;
	}
	gpseudo(ADATA, s, D_TREE, (long)a);
	p->from.u0.s0.offset += o;
	p->from.u0.s0.displace = w;
}
