...
xcom(Node *n)
{
	...
	case OADD:
		...
		if(n->addable == 8) {
			...
			l->scale = idx.scale;
			l->addable = 9;
			l->complex = l->u0.s0.nright->complex;
			l->type = l->u0.s0.nleft->type;
			n->op = OADDR;
			n->u0.s0.nleft = l;
			n->u0.s0.nright = Z;
			n->addable = 0;
	...
	aseae:		/* hack that there are no byte/short mul/div operators */
		if(n->type->etype == TCHAR || n->type->etype == TSHORT) {
			...
			n->u0.s0.nright->type = types[TLONG];
			n->type = types[TLONG];
		}
		if(n->type->etype == TUCHAR || n->type->etype == TUSHORT) {
			n->u0.s0.nright = new1(OCAST, n->u0.s0.nright, Z);
			n->u0.s0.nright->type = types[TULONG];
			n->type = types[TULONG];
		}
		goto asop;
...
loop:
	...
	if(l->u0.s0.nright->addable == 20) {
		idx.regtree = l->u0.s0.nleft;
		idx.scale = l->u0.s0.nright->u0.nvconst;
	} else {
		idx.regtree = l->u0.s0.nright;
		idx.scale = l->u0.s0.nleft->u0.nvconst;
	}
