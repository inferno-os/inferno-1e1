 ...
 *	-p		use standard /bin/cpp ANSI preprocessor
 *			-p is NOT SUPPORTED FOR Nt.
 */
...
child:
		...
		if(debug['p']) {
			diag(Z, "The -p option is not supported on Nt");
		} else {
			if(argc < 1)
				newfile("stdin", 0);
			else
...
/*
 * This function is a "safe" method of adding non-duplicate
 * directories to header file search path. If the include
 * array is full, an error is printed out and the program
 * exits. If the directory to be added is a already in the array,
 * nothing is done. Otherwise, the directory is added to the include
 * array.
 */
void
setinclude(char *p)
{
	...
	if(p == 0)
		return;
	for(ii=0; ii < ninclude; ii++) {
		if(strcmp(p, include[ii])==0)
			return;
	}
