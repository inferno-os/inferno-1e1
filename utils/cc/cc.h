...
struct	Node
{
	union
	{
		...
		double	nfconst;		/* fp constant */
		vlong	nvconst;		/* non fp const */
		char*	ncstring;	/* character string */
		ushort*	nrstring;	/* rune string */
	} u0;
	Sym*	sym;
	Type*	type;
	long	lineno;
...
extern	char	*onames[], *tnames[], *gnames[], *cnames[], *qnames[];
...
Sym*	lookup(void);
void	main(int, char*[]);
int	mygetc(void);	/* Local definition of getc() */
