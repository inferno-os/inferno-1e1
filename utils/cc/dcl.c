...
mkstatic(Sym *s)
{
	...
	if(s->class != CLOCAL)
		return s;
	/*
	 * The type cast is used to guarantee that all implementations
	 * of this compiler will come up with the same static names.
	 * Without the typecast, a block value of 0x81 may become
	 * -127 on some systems and 129 on other systems.
	 */
	snprint(symb, NSYMB, "%s$%d", s->name, (signed char)s->block);
	s1 = lookup();
	if(s1->class != CSTATIC) {
		s1->type = s->type;
		s1->offset = s->offset;
...
loop:
	...
	if(a->op == OUSED) {
		...
		b->type = a->type->link;
		if(a->op == OSTRING) {
			b->u0.nvconst = convvtox(*a->u0.ncstring, TCHAR);
			a->u0.ncstring++;
		}
		if(a->op == OLSTRING) {
			b->u0.nvconst = convvtox(*a->u0.nrstring, TUSHORT);
			a->u0.nrstring++;
		}
...
loop:
	...
	switch(n->op) {
	case OLIST:
		tl = ofnproto(n->u0.s0.nleft);
		tr = ofnproto(n->u0.s0.nright);
		if(tl == T)
			return tr;
		tl->down = tr;
		return tl;
...
argmark(Node *n, int pass)
{
	...
	stkoff = 0;
	for(; n->u0.s0.nleft != Z; n = n->u0.s0.nleft) {
		if(n->op != OFUNC || n->u0.s0.nleft->op != ONAME)
			continue;
		walkparam(n->u0.s0.nright, pass);
		if(pass != 0 && anyproto(n->u0.s0.nright) == OLDPROTO) {
			t = typ(TFUNC, n->u0.s0.nleft->sym->type->link);
			t->down = typ(TOLD, T);
			t->down->down = ofnproto(n->u0.s0.nright);
			tmerge(t, n->u0.s0.nleft->sym);
			n->u0.s0.nleft->sym->type = t;
		}
		break;
	}
	autoffset = 0;
...
walkparam(Node *n, int pass)
{
	...
	if(n != Z && n->op == OPROTO && n->u0.s0.nleft == Z && n->type == types[TVOID])
		return;
...
loop:
	...
	case OLIST:
		walkparam(n->u0.s0.nleft, pass);
		n = n->u0.s0.nright;
		goto loop;

	case OPROTO:
		for(n1 = n; n1 != Z; n1=n1->u0.s0.nleft)
			if(n1->op == ONAME) {
				if(pass == 0) {
					s = n1->sym;
					push1(s);
				...
				dodecl(pdecl, CPARAM, n->type, n->u0.s0.nleft);
				break;
			}
		if(n1)
			break;
		...
		dodecl(NODECL, CPARAM, n->type, n->u0.s0.nleft);
...
fnproto(Node *n)
{
	...
	r = anyproto(n->u0.s0.nright);
	if(r == 0 || (r & OLDPROTO)) {
		if(r & ANSIPROTO)
			diag(n, "mixed ansi/old function declaration: %F", n->u0.s0.nleft);
		return T;
	}
	return fnproto1(n->u0.s0.nright);
}
...
fnproto1(Node *n)
{
	...
	switch(n->op) {
	case OLIST:
		t = fnproto1(n->u0.s0.nleft);
		if(t != T)
			t->down = fnproto1(n->u0.s0.nright);
		return t;

	case OPROTO:
		lastdcl = T;
		dodecl(NODECL, CXXX, n->type, n->u0.s0.nleft);
		t = typ(TXXX, T);
		if(lastdcl != T)
...
doenum(Sym *s, Node *n)
{
	...
	if(n) {
		...
		if(!typefd[en.cenum->etype])
			en.lastenum = n->u0.nvconst;
		else
			en.floatenum = n->u0.nfconst;
	}
	if(dclstack)
		push1(s);
	...
	if(!typefd[s->tenum->etype]) {
		s->u0.svconst = convvtox(en.lastenum, s->tenum->etype);
		en.lastenum++;
	} else {
		s->u0.sfconst = en.floatenum;
		en.floatenum++;
	}
...
symadjust(Sym *s, Node *n, long del)
{
	...
	default:
		if(n->u0.s0.nleft)
			symadjust(s, n->u0.s0.nleft, del);
		if(n->u0.s0.nright)
			symadjust(s, n->u0.s0.nright, del);
		return;
...
contig(Sym *s, Node *n, long v)
{
	...
	if(w <= 4)
		goto no;
	if(n->op == OAS)
		if(n->u0.s0.nleft->type)
		if(n->u0.s0.nleft->type->width == w)
			goto no;
	while(w & 3)
		w++;	/* is this a bug?? */
/*
 ...
 */
	...
	for(q=n; q->op != ONAME; q=q->u0.s0.nleft)
		;

	p = new(ONAME, Z, Z);
	*p = *q;
	p->type = typ(TIND, types[TLONG]);
	p->u0.s2.noffset = s->offset;
