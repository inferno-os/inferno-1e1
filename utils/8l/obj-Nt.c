...
zaddr(uchar *p, Adr *a, Sym *h[])
{
	...
	a->u0.aoffset = 0;
	if(t & T_OFFSET) {
		a->u0.aoffset = p[c] | (p[c+1]<<8) | (p[c+2]<<16) | (p[c+3]<<24);
		c += 4;
	}
	a->u1.asym = S;
	if(t & T_SYM) {
	...
	a->type = D_NONE;
	if(t & T_FCONST) {
		a->u0.aieee.l = p[c] | (p[c+1]<<8) | (p[c+2]<<16) | (p[c+3]<<24);
		a->u0.aieee.h = p[c+4] | (p[c+5]<<8) | (p[c+6]<<16) | (p[c+7]<<24);
		c += 8;
		a->type = D_FCONST;
	} else
	if(t & T_SCONST) {
...
loop:
	...
	if(r < 100 && r < c) {		/* enough for largest prog */
		bsize = readsome(f, buf.s0.bxbuf, bloc, bsize, c);
		if(bsize == 0)
			goto eof;
		bloc = buf.s0.bxbuf;
		goto loop;
	...
	case ADYNT:
		...
		p->from.u1.asym->type = SDATA;
		if(curtext == P) {
			diag("DYNT not in text: %P\n", p);
			break;
		}
		p->to.u1.asym = curtext->from.u1.asym;
		p->to.type = D_ADDR;
		p->to.index = D_EXTERN;
		goto data;
...
/*
 * realloc() is called by Nt's atexit(), so comment this one
 * out to make sure that Nt's realloc() is invoked.
 *
 *void*
 *realloc(void *p, uint n)
 *{
 *	fprint(2, "realloc-Nt called\n", p, n);
 *	abort();
 *	return 0;
 *}
 */
