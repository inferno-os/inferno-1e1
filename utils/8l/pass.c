	...
	for(p = datap; p != P; p = p->link) {
		...
		if(s->type == 0 || s->type == SXREF)
			diag("undefined %s initializer of %s\n",
				s->name, p->from.u1.asym->name);
		p->to.u0.aoffset += s->value;
		p->to.type = D_CONST;
		if(s->type == SDATA || s->type == SBSS)
			p->to.u0.aoffset += INITDAT;
	}
}
	...
	for(p = firstp; p != P; p = p->link) {
		...
		case ACALL:
			if(curtext != P && curtext->from.u1.asym != S && curtext->to.u0.aoffset >= 0) {
				f = maxbecome - curtext->from.u1.asym->frame;
				if(f <= 0)
					break;
				/* calling a become or calling a variable */
				if(p->to.u1.asym == S || p->to.u1.asym->become) {
					curtext->to.u0.aoffset += f;
					if(debug['b']) {
						curp = p;
	...
	for(p = firstp; p != P; p = p->link) {
		if(p->as == ATEXT) {
			...
			if(autoffset) {
				...
				p->as = AADJSP;
				p->from.type = D_CONST;
				p->from.u0.aoffset = autoffset;
			}
			deltasp = autoffset;
		}
		a = p->from.type;
		if(a == D_AUTO)
			p->from.u0.aoffset += deltasp;
		if(a == D_PARAM)
			p->from.u0.aoffset += deltasp + 4;
		a = p->to.type;
		if(a == D_AUTO)
			p->to.u0.aoffset += deltasp;
		if(a == D_PARAM)
			p->to.u0.aoffset += deltasp + 4;
