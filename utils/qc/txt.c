...
regaalloc1(Node *n, Node *nn)
{
	...
	nodreg(n, nn, r);
	reg[r]++;
	curarg += 4;	/* BUG? must allow 8 for each to keep alignment? */
	if(cursafe+curarg > maxargsafe)
		maxargsafe = cursafe+curarg;
}
...
raddr(Node *n, Prog *p)
{
	...
	naddr(n, &a);
	if(R0ISZERO && a.type == D_CONST && a.u0.aoffset == 0) {
		a.type = D_REG;
		a.reg = REGZERO;
	}
...
gopcode(int o, Node *f1, Node *f2, Node *t)
{
	...
	if(f2 != Z) {
		naddr(f2, &ta);
		p->reg = ta.reg;
		if(ta.type == D_CONST && ta.u0.aoffset == 0) {
			if(R0ISZERO)
				p->reg = REGZERO;
			else
