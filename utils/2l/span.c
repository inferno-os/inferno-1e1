...
andsize(Prog *p, Adr *ap)
{
	...
	if(ap->index != D_NONE) {
		...
		case D_STATIC:
		case D_EXTERN:
			if(ap->u1.sym->type == STEXT)
				return n+4;	/* see below */
			v = ap->u1.sym->value + ap->u0.s0.offset - A6OFFSET;
			if(debug['6'])
				v += INITDAT + A6OFFSET;
		}
		if(v != 0) {
...
asmsym(void)
{
	...
	for(p=textp; p!=P; p=p->cond) {
		...
		/* filenames first */
		for(a=p->to.u1.autom; a; a=a->link)
			if(a->type == D_FILE)
...
asmsp(void)
{
	...
	oldsp = 0;
	for(p = firstp; p != P; p = p->link) {
		if(p->u0.stkoff == oldsp || p->as == ATEXT || p->as == ANOP) {
			if(p->as == ATEXT)
				curtext = p;
			if(debug['G'])
