...
Dconv(va_list *arg, Fconv *fp)
{
	...
	case D_BRANCH:
		if(bigP != P && bigP->cond != P)
			...
			sprint(str, "%ld(PC)", a->u0.s0.offset);
		break;

	case D_EXTERN:
		sprint(str, "%s+%ld(SB)", a->u1.sym->name, a->u0.s0.offset);
		break;

	case D_STATIC:
		sprint(str, "%s<%d>+%ld(SB)", a->u1.sym->name,
			a->u1.sym->version, a->u0.s0.offset);
		break;

	case D_AUTO:
		sprint(str, "%s+%ld(SP)", a->u1.sym->name, a->u0.s0.offset);
		break;

	case D_PARAM:
		if(a->u1.sym)
			sprint(str, "%s+%ld(FP)", a->u1.sym->name, a->u0.s0.offset);
		else
			sprint(str, "%ld(FP)", a->u0.s0.offset);
		break;
	...
	case D_QUICK:
		sprint(str, "$Q%ld", a->u0.s0.offset);
		break;

	case D_FCONST:
		sprint(str, "$(%.8lux,%.8lux)", a->u0.ieee.h, a->u0.ieee.l);
		goto out;

	case D_SCONST:
		sprint(str, "$\"%S\"", a->u0.scon);
		goto out;
	}
	if(a->u0.s0.displace) {
...
diag(char *fmt, ...)
{
	...
	tn = "??none??";
	if(curtext != P && curtext->from.u1.sym != S)
		tn = curtext->from.u1.sym->name;
	va_start(arg, fmt);
