...
zaddr(uchar *p, Adr *a, Sym *h[])
{
	...
	if(t == T_OFFSET) {
		a->index = D_NONE;
		a->u0.s0.offset = p[1] | (p[2]<<8) | (p[3]<<16) | (p[4]<<24);
		a->type = p[5];
		return 6;
	}
	if(t == (T_OFFSET|T_SYM)) {
	...
	if(t == (T_INDEX|T_OFFSET|T_SYM)) {
		a->index = p[1] | (p[2]<<8);
		a->scale = p[3];
		a->u0.s0.displace = p[4] | (p[5]<<8) | (p[6]<<16) | (p[7]<<24);
		a->u0.s0.offset = p[8] | (p[9]<<8) | (p[10]<<16) | (p[11]<<24);
		s = h[p[12]];
		a->u1.sym = s;
		a->type = p[13];
		c = 14;
	...
	if(t & T_INDEX) {
		a->index = p[c] | (p[c+1]<<8);
		a->scale = p[c+2];
		a->u0.s0.displace = p[c+3] | (p[c+4]<<8) | (p[c+5]<<16) | (p[c+6]<<24);
		c += 7;
	}
	if(t & T_OFFSET) {
		a->u0.s0.offset = p[c] | (p[c+1]<<8) | (p[c+2]<<16) | (p[c+3]<<24);
		c += 4;
	}
	if(t & T_SYM) {
		a->u1.sym = h[p[c]];
	...
	if(t & T_FCONST) {
		a->u0.ieee.l = p[c] | (p[c+1]<<8) | (p[c+2]<<16) | (p[c+3]<<24);
		a->u0.ieee.h = p[c+4] | (p[c+5]<<8) | (p[c+6]<<16) | (p[c+7]<<24);
		c += 8;
		a->type = D_FCONST;
	} else
	if(t & T_SCONST) {
...
loop:
	...
	if(r < 100 && r < c) {		/* enough for largest prog */
		bsize = readsome(f, buf.s0.xbuf, bloc, bsize, c);
		if(bsize == 0)
			goto eof;
		bloc = buf.s0.xbuf;
		goto loop;
	...
	switch(p->as) {
	case AHISTORY:
		if(p->to.u0.s0.offset == -1) {
			addlib(p->line);
			histfrogp = 0;
			goto loop;
		}
		addhist(p->line, D_FILE);		/* 'z' */
		if(p->to.u0.s0.offset)
			addhist(p->to.u0.s0.offset, D_FILE1);	/* 'Z' */
		histfrogp = 0;
		goto loop;
	...
	case AGLOBL:
		...
		if(p->to.u0.s0.offset > s->value)
			s->value = p->to.u0.s0.offset;
		goto loop;
	...
	case ALSRW:
		if(p->from.type == D_CONST)
		if(p->from.u0.s0.offset > 0)
		if(p->from.u0.s0.offset <= 8)
			p->from.type = D_QUICK;
		goto casdef;
		...
		if(optab[p->as].fas != AXXX) {
			...
			if(dv >= -(1L<<30) && dv <= (1L<<30)) {
				...
				if(lv == dv) {
					...
					p->from.type = D_CONST;
					p->from.u0.s0.offset = lv;
					p->from.u0.s0.displace = 0;
				}
			}
		}
		if(p->to.type == D_BRANCH)
			p->to.u0.s0.offset += ipc;
		lastp->link = p;
		lastp = p;
		p->pc = pc;
		pc++;
...
doprof1(void)
{
	...
	for(p = firstp->link; p != P; p = p->link) {
		if(p->as == ATEXT) {
			...
			p->link = q;
			q->from.type = D_CONST;
			q->from.u0.s0.offset = 1;
			q->to.type = D_EXTERN;
			q->to.u1.sym = s;
			q->to.u0.s0.offset = n*4 + 4;
			...
			q->from.u1.sym = s;
			q->from.u0.s0.offset = n*4;
			q->from.u0.s0.displace = 4;
			q->to.type = D_EXTERN;
			q->to.u1.sym = p->from.u1.sym;
			n += 2;
			continue;
		}
	}
