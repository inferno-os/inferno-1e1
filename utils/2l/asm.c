...
asmins(Prog *p)
{
	...
	case 33:	/* bcase */
		...
		q->as = ADATA;
		q->to.type = D_CONST;
		q->to.u0.s0.offset = p->cond->pc - casepc - 2;
		q->from.u0.s0.displace = 2;
		q->link = datap;
		datap = q;
		if(debug['a'])
			Bprint(&bso, "%P\n", q);
...
asmea(Prog *p, Adr *a)
{
	...
	case I_ADDR|D_EXTERN:
		...
		v = a->u1.sym->value + a->u0.s0.offset;
		if(t != STEXT)
			v += INITDAT;
	...
	case D_FCONST:
		r = f? o->srcsp: o->dstsp;
		for(i=0; i<r; i++)
			((char*)op)[i] = gnuxi(&a->u0.ieee, i, r);
		op += r/2;
		return (7<<3) | 4;

	case D_QUICK:
		v = a->u0.s0.offset & 0xff;
		return 0110 | (v<<7);
	...
	case D_EXTERN:
		...
		v = a->u1.sym->value + a->u0.s0.offset - A6OFFSET;
		if(debug['6']) {
			v += INITDAT + A6OFFSET;
			goto adr;
		}
...
cflush(void)
{
	...
	n = sizeof(buf.s0.cbuf) - cbc;
	if(n)
		write(cout, buf.s0.cbuf, n);
	cbp = buf.s0.cbuf;
...
datblk(long s, long n)
{
	...
	for(p = datap; p != P; p = p->link) {
		curp = p;
		l = p->from.u1.sym->value + p->from.u0.s0.offset - s;
		c = p->from.u0.s0.displace;
		i = 0;
		if(l < 0) {
			if(l+c <= 0)
				continue;
		...
		case D_SCONST:
			if(debug['a'] && i == 0) {
				Bprint(&bso, "%lux:%P\n\t\t", l+s+INITDAT, curp);
				for(j=0; j<c; j++)
					Bprint(&bso, "%.2ux", p->to.u0.scon[j] & 0xff);
				Bprint(&bso, "\n", curp);
			}
			for(; i<c; i++) {
				buf.dbuf[l] = p->to.u0.scon[i];
				l++;
			}
			break;
		default:
			fl = p->to.u0.s0.offset;
			if(p->to.u1.sym) {
				if(p->to.u1.sym->type == STEXT)
					fl += p->to.u1.sym->value;
				if(p->to.u1.sym->type == SDATA)
					fl += p->to.u1.sym->value + INITDAT;
				if(p->to.u1.sym->type == SBSS)
					fl += p->to.u1.sym->value + INITDAT;
			}
