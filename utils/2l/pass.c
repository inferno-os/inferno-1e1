	...
	for(p = datap; p != P; p = p->link) {
		...
		if(s->type != SDATA)
			diag("initialize non-data (%d): %s\n%P\n",
				s->type, s->name, p);
		t = p->from.u0.s0.offset + p->from.u0.s0.displace;
		if(t > s->value)
...
follow(void)
{
	...
	for(p = firstp; p != P; p = p->link) {
		...
		p->u0.stkoff = -1;	/* initialization for stkoff */
		if(p->as == ATEXT) {
			p->u0.stkoff = 0;
			o = p->to.u0.s0.offset;
			continue;
		}
		if(p->as == AADJSP && p->from.u0.s0.offset == 0) {
			p->u0.stkoff = o;
			continue;
		}
	}
...
patch(void)
{
	...
	for(p = firstp; p != P; p = p->link) {
		if(p->as == ATEXT)
			curtext = p;
		if((p->as == ABSR || p->as == ARTS) && p->to.u1.sym != S) {
			s = p->to.u1.sym;
			if(s->type != STEXT) {
...
dostkoff(void)
{
	...
	for(p = firstp; p != P; p = p->link) {
		...
		if(a == D_AUTO)
			p->from.u0.s0.offset += p->u0.stkoff;
		if(a == D_PARAM)
			p->from.u0.s0.offset += p->u0.stkoff + 4;
		a = p->to.type & D_MASK;
		if(a == D_AUTO)
			p->to.u0.s0.offset += p->u0.stkoff;
		if(a == D_PARAM)
			p->to.u0.s0.offset += p->u0.stkoff + 4;
		if(p->as != ARTS)
			continue;
		if(p->u0.stkoff == 0)
			continue;
		...
		q->as = AADJSP;
		q->from.type = D_CONST;
		q->from.u0.s0.offset = -q->u0.stkoff;
	}
}
