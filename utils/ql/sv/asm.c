...
asmb(void)
{
	...
	case 5:
		...
		strnput("", 9);
		lput((2L<<16)|20L);		/* type = EXEC; machine = PPC */
		lput(1L);			/* version = CURRENT */
		lput(0xF0004000);		/* entry vaddr */
		lput(52L);			/* offset to first phdr */
		lput(0L);			/* offset to first shdr */
		lput(0L);			/* flags = MIPS */
		lput((52L<<16)|32L);		/* Ehdr & Phdr sizes*/
		lput((1L<<16)|40L);		/* # Phdrs & Shdr size */
		lput((0L<<16)|8L);		/* # Shdrs & shdr string size */

		lput(1L);			/* text - type = PT_LOAD */
		lput(HEADR);			/* file offset */
		lput(0xF0004000);		/* vaddr */
		lput(0xF0004000);		/* paddr */
		lput(textsize+datsize);		/* file size */
		lput(textsize+datsize+bsssize);		/* memory size */
		lput(0x07L);			/* protections = RWX */
		lput(0x1000L);			/* alignment code?? */
		break;
	case 6:
		/* no header */
		break;
