 ...
 *	-H5 -T0x4000 -R0		is first attempt at ppc32-ELF boot image
 *	-H6 -T0x4000 -R4		is raw executable, no header
 */
...
main(int argc, char *argv[])
{
	...
	case 4:	/* aix unix xcoff executable */
		...
		if(INITRND == -1)
			INITRND = 0;
		break;
	case 5:	/* possibly ibox unix elf bootable */
		...
		if(INITRND == -1)
			INITRND = 0;
		break;
	case 6:	/* headerless format, in ibox desperation */
		HEADR = 0;
		if(INITTEXT == -1)
			INITTEXT = 0x004000L+HEADR;
		if(INITDAT == -1)
			INITDAT = 0;
		if(INITRND == -1)
			INITRND = 4;
...
zaddr(uchar *p, Adr *a, Sym *h[])
{
	...
	case D_OREG:
	case D_CONST:
		a->u0.aoffset = p[4] | (p[5]<<8) |
			(p[6]<<16) | (p[7]<<24);
		c += 4;
		break;

	case D_SCONST:
		memmove(a->u0.asval, p+4, NSNAME);
		c += NSNAME;
		break;

	case D_FCONST:
		a->u0.aieee.l = p[4] | (p[5]<<8) |
			(p[6]<<16) | (p[7]<<24);
		a->u0.aieee.h = p[8] | (p[9]<<8) |
			(p[10]<<16) | (p[11]<<24);
		c += 8;
		break;
	}
...
loop:
	...
	if(r < 100 && r < c) {		/* enough for largest prog */
		bsize = readsome(f, buf.u0.bxbuf, bloc, bsize, c);
		if(bsize == 0)
			goto eof;
		bloc = buf.u0.bxbuf;
		goto loop;
	...
	case AHISTORY:
		...
		if(p->to.u0.aoffset)
			addhist(p->to.u0.aoffset, D_FILE1);	/* 'Z' */
		histfrogp = 0;
		goto loop;
	...
	case AGLOBL:
		...
		if(p->to.u0.aoffset > s->value)
			s->value = p->to.u0.aoffset;
		break;
	...
	case ATEXT:
		...
		curtext = p;
		autosize = (p->to.u0.aoffset+3L) & ~3L;
		p->to.u0.aoffset = autosize;
		autosize += 4;
		s = p->from.sym;
		if(s == S) {
	...
	case AFMOVS:
		...
		if(p->from.type == D_FCONST) {
			/* size sb 9 max */
			sprint(literal, "$%lux", ieeedtof(&p->from.u0.aieee));
			s = lookup(literal, 0);
			if(s->type == 0) {
				s->type = SBSS;
				s->value = 4;
	...
	case AFMOVD:
		...
		if(p->from.type == D_FCONST) {
			...
			sprint(literal, "$%lux.%lux",
				p->from.u0.aieee.l, p->from.u0.aieee.h);
			s = lookup(literal, 0);
			if(s->type == 0) {
				s->type = SBSS;
				s->value = 8;
	...
	case ASUBC:
		if(p->from.type == D_CONST) {
			p->from.u0.aoffset = -p->from.u0.aoffset;
			p->as = AADDC;
		}
		goto casedef;
