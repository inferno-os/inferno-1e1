...
ginit(void)
{
	...
	fregnode0 = regnode;
	fregnode0.u0.s2.nreg = D_F0;
	fregnode0.type = types[TDOUBLE];

	fregnode1 = fregnode0;
	fregnode1.u0.s2.nreg = D_F0+1;
...
naddr(Node *n, Adr *a)
{
	...
	case OADD:
		if(n->u0.s0.nright->op == OCONST) {
			v = n->u0.s0.nright->u0.nvconst;
			naddr(n->u0.s0.nleft, a);
		} else
		if(n->u0.s0.nleft->op == OCONST) {
			v = n->u0.s0.nleft->u0.nvconst;
 ...
 *****/
	...
	case CASE(	TSHORT,	TIND):
		...
		if(f->op == OCONST) {
			f->u0.nvconst &= 0xffff;
			if(f->u0.nvconst & 0x8000)
				f->u0.nvconst |= 0xffff0000;
			a = AMOVL;
		}
		break;
	...
	case CASE(	TCHAR,	TIND):
		...
		if(f->op == OCONST) {
			f->u0.nvconst &= 0xff;
			if(f->u0.nvconst & 0x80)
				f->u0.nvconst |= 0xffffff00;
			a = AMOVL;
		}
		break;
...
gins(int a, Node *f, Node *t)
{
	...
	if(f != Z && f->op == OINDEX) {
		...
		v = constnode.u0.nvconst;
		cgen(f->u0.s0.nright, &nod);
		constnode.u0.nvconst = v;
		idx.reg = nod.u0.s2.nreg;
		regfree(&nod);
	}
	if(t != Z && t->op == OINDEX) {
		regalloc(&nod, &regnode, Z);
		v = constnode.u0.nvconst;
		cgen(t->u0.s0.nright, &nod);
		constnode.u0.nvconst = v;
		idx.reg = nod.u0.s2.nreg;
		regfree(&nod);
	}
...
/* BOTCH put this code in peep
		if(f->op == OCONST &&
		   ...
		   t->op != OIND && t->op != OINDEX) {
			if(f->u0.nvconst == 0xffff) {
				gins(AMOVWLZX, t, t);
				return;
			}
			if(f->u0.nvconst == 0xff) {
				gins(AMOVBLZX, t, t);
				return;
			}
		}
