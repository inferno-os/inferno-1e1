...
cgen(Node *n, Node *nn)
{
	...
	case OASHR:
		...
		reg[D_CX]++;
		if(nn->op == OREGISTER && nn->u0.s2.nreg == D_CX)
			regalloc(&nod1, l, Z);
		else
			regalloc(&nod1, l, nn);
	...
	case ODIV:
		...
		if(nodreg(&nod, nn, D_AX)) {
			...
			if(isreg(l, D_AX)) {
				nod3 = *n;
				nod3.u0.s0.nleft = &nod2;
				cgen(&nod3, nn);
			} else
			if(isreg(r, D_AX)) {
				nod3 = *n;
				nod3.u0.s0.nright = &nod2;
				cgen(&nod3, nn);
			} else
				cgen(n, nn);
	...
	asbitop:
		...
		if(typefd[nod3.type->etype])
			...
			onod.u0.s0.nleft = &nod4;
			onod.u0.s0.nright = &nod3;
			cgen(&onod, Z);
		}
		regfree(&nod3);
		gmove(&nod4, &nod);
	...
	case OFUNC:
		if(l->complex >= FNX) {
			...
			regret(&nod, l->u0.s0.nleft);
			cgen(l->u0.s0.nleft, &nod);
			regsalloc(&nod1, l->u0.s0.nleft);
			gmove(&nod, &nod1);
			regfree(&nod);

			nod = *n;
			nod.u0.s0.nleft = &nod2;
			nod2 = *l;
			nod2.u0.s0.nleft = &nod1;
			nod2.complex = 1;
			cgen(&nod, nn);
	...
	case ODOT:
		...
		if(nn != Z) {
			...
			nod.u0.s2.noffset += (long)r->u0.nvconst;
			nod.type = n->type;
			cgen(&nod, nn);
		}
		break;
	...
	case OCOND:
		bcgen(l, 1);
		p1 = p;
		cgen(r->u0.s0.nleft, nn);
		gbranch(OGOTO);
		patch(p1, pc);
		p1 = p;
		cgen(r->u0.s0.nright, nn);
		patch(p1, pc);
		break;
...
lcgen(Node *n, Node *nn)
{
	...
	case OCOMMA:
		cgen(n->u0.s0.nleft, n->u0.s0.nleft);
		lcgen(n->u0.s0.nright, nn);
		break;

	case OIND:
		cgen(n->u0.s0.nleft, nn);
		break;

	case OCOND:
		bcgen(n->u0.s0.nleft, 1);
		p1 = p;
		lcgen(n->u0.s0.nright->u0.s0.nleft, nn);
		gbranch(OGOTO);
		patch(p1, pc);
		p1 = p;
		lcgen(n->u0.s0.nright->u0.s0.nright, nn);
		patch(p1, pc);
		break;
	}
}
...
boolgen(Node *n, int true, Node *nn)
{
	...
	case OCOND:
		bcgen(l, 1);
		p1 = p;
		bcgen(r->u0.s0.nleft, true);
		p2 = p;
		gbranch(OGOTO);
		patch(p1, pc);
		p1 = p;
		bcgen(r->u0.s0.nright, !true);
		patch(p2, pc);
		p2 = p;
		gbranch(OGOTO);
		patch(p1, pc);
...
sugen(Node *n, Node *nn, long w)
{
	...
	case OCONST:
		if(n->type && typev[n->type->etype]) {
			...
			gmove(nodconst((long)(n->u0.nvconst)), &nod1);
			nod1.u0.s2.noffset += SZ_LONG;
			gmove(nodconst((long)(n->u0.nvconst>>32)), &nod1);
	...
	case ODOT:
		...
		if(nn != Z) {
			...
			nod1.u0.s2.noffset += (long)r->u0.nvconst;
			nod1.type = n->type;
			sugen(&nod1, nn, w);
		}
		break;
	...
	case OSTRUCT:
		...
		for(t = n->type->link; t != T; t = t->down) {
			...
			nod0.op = OAS;
			nod0.type = t;
			nod0.u0.s0.nleft = &nod1;
			nod0.u0.s0.nright = 0;

			nod1 = znode;
			nod1.op = OIND;
			nod1.type = t;
			nod1.u0.s0.nleft = &nod2;

			nod2 = znode;
			nod2.op = OADD;
			nod2.type = typ(TIND, t);
			nod2.u0.s0.nleft = &nod3;
			nod2.u0.s0.nright = &nod4;
	...
	case OAS:
		if(nn == Z) {
			if(n->addable < INDEXED)
				sugen(n->u0.s0.nright, n->u0.s0.nleft, w);
			break;
		}
		/* BOTCH -- functions can clobber rathole */
		sugen(n->u0.s0.nright, nodrat, w);
		warn(n, "non-interruptable temporary");
		sugen(nodrat, n->u0.s0.nleft, w);
		sugen(nodrat, nn, w);
		break;
	...
	case OFUNC:
		...
		if(nn->op != OIND) {
			...
			nn = nn->u0.s0.nleft;
		n = new(OFUNC, n->u0.s0.nleft, new(OLIST, nn, n->u0.s0.nright));
		n->type = types[TVOID];
		n->u0.s0.nleft->type = types[TVOID];
		cgen(n, Z);
		break;
	...
	case OCOND:
		bcgen(n->u0.s0.nleft, 1);
		p1 = p;
		sugen(n->u0.s0.nright->u0.s0.nleft, nn, w);
		gbranch(OGOTO);
		patch(p1, pc);
		p1 = p;
		sugen(n->u0.s0.nright->u0.s0.nright, nn, w);
		patch(p1, pc);
		break;

	case OCOMMA:
		cgen(n->u0.s0.nleft, Z);
		sugen(n->u0.s0.nright, nn, w);
		break;
	}
	return;
