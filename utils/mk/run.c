...
static int pidslot(int);
	...
	This code is not portable.  The call to exec assumes
	&a turns the argument list into a vector, which is not
	true on some machines.
		...
		if(pid == 0){
			...
			if(pid != 0){
				...
				if(j->r->attr&NOMINUSE)
					...
					execl(shell, shellname,  (char *)0);
				else
					execl(shell, shellname, "-e", (char *)0);
				perror(shell);
				_exits("exec");
			} else {
				int k;
...
/*	Initializers must be in the enum order above.
 */
...
dovars(Job *j, int slot)
{
	...
	if(j->r->attr&REGEXP) {			/* memory leak */
		if (j->match[1].s.sp && j->match[1].e.ep) {
			s = j->match[1].e.ep+1;
			c = *s;
			*s = 0;
			myenv[STEM].w.s = strdup(j->match[1].s.sp);
			*s = c;
		} else
			myenv[STEM].w.s = "";
	} else
	...
	for(i = 0; i <= 9; i++){
		if((j->r->attr&REGEXP) && j->match[i].s.sp && j->match[i].e.ep) {
			s = j->match[i].e.ep;
			c = *s;
			*s = 0;
			mp->w.s = strdup(j->match[i].s.sp);	/*leak*/
			*s = c;
