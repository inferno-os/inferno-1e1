...
direct:
	...
	gmove(nodconst((q+nc-1)->val - v), tn);
	gopcode(OLO, tn, n, Z);
	patch(p, def);
	gopcode(OEQ, n, Z, Z);
	p->as = ACASE;
	for(i=0; i<nc; i++) {
		if(debug['W'])
			print("case = %.8lux\n", q->val);
		while(q->val != v) {
			gopcode(OEQ, nodconst(-1), Z, Z);
			p->as = ABCASE;
			patch(p, def);
			v++;
		}
		gopcode(OEQ, nodconst(v), Z, Z);
		p->as = ABCASE;
		patch(p, q->label);
		q++;
		v++;
...
mulcon(Node *n, Node *nn)
{
	...
	if(r->op != OCONST)
		return 0;
	v = convvtox(r->u0.nvconst, n->type->etype);
	if(v != r->u0.nvconst) {
		if(debug['M'])
			print("%L multiply conv: %lld\n", n->lineno, r->u0.nvconst);
		return 0;
	}
	m = mulcon0(v);
	if(!m) {
...
gextern(Sym *s, Node *a, long o, long w)
{
	...
	if(a->op == OCONST && typev[a->type->etype]) {
		gpseudo(ADATA, s, nod32const(a->u0.nvconst>>32));
		p->from.u0.aoffset += o;
		p->reg = 4;
		gpseudo(ADATA, s, nod32const(a->u0.nvconst));
