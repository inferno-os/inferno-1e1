...
main(int argc, char *argv[])
{
	...
	fmtinstall('N', nameconv);		/* decl name */
	fmtinstall('O', opconv);
	fmtinstall('g', gfltconv);
	fmtinstall('Q', ntconv);		/* src expression with type */
...
strtoi(char *t, int base)
{
	...
	for(s = t; c = *s; s++){
		if(map[c] & Mdigit)
			c -= '0';
		else if(map[c] & Mlower)
			c = c - 'a' + 10;
		else if(map[c] & Mupper)
			c = c - 'A' + 10;
		if(c >= base){
...
lexnum(int c)
{
	...
	enum { Int, Radix, RadixSeen, Frac, ExpSeen, ExpSignSeen, Exp } state;
	double d;
	Long v;
	int i;
