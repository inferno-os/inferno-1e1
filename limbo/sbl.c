...
sbltype(Type *t)
{
	...
	case Tadt:
		if(t->decl->ty->sbl < 0)
			fatal("no sbl for type %T", t);
		Bprint(bsym, "%c%d.%d\n", sbltname[t->kind], t->size, t->decl->ty->sbl);
		break;
	case Tmodule:
		Bprint(bsym, "%c%s\n%s", sbltname[t->kind], t->decl->sym->name, sblsrcconv(buf, buf+sizeof(buf), &t->decl->src));
		break;
	}
}
