...
static void encode(uchar*, ulong*, ulong);
static void decode(ulong*, uchar*, ulong);

static void
shablock(uchar *p, ulong len, SHAstate *s)
{
	ulong a, b, c, d, e, tmp;
	uchar *end;
	ulong *wp, *wend;
 ...
 *  to pad.
 *
 *  we assume ulong is 32 bits
 */
SHAstate*
sha(uchar *p, ulong len, uchar *digest, SHAstate *s)
{
	uchar buf[128];
	ulong x[16];
	int i;
 ...
 */
static void
decode(ulong *output, uchar *input, ulong len)
{
	uchar *e;
