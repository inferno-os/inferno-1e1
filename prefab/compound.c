...
redrawcompound(Image *i, Rectangle clipr, Prefab_Compound *c)
{
	...
	if(elemcolor==nil || edgecolor==nil)
		return;
	draw(i, r, elemcolor, i->display->ones, r.min);
	if(lookupelement(c->title) != H){
		...
		if(c->title->environ!=H && c->title->environ->style!=H && rectXrect(r, rt)){
			...
			case ETitle:
				...
				if(font != nil)
					len = 2+1+stringwidth(font, stringchk(c->title->str));
				break;
			case EVertical:
				font = lookupfont(s->titlefont);
				if(font != nil)
					for(list=c->title->kids; list!=H; list=list->tail){
						...
						l = stringwidth(font, stringchk(e->str));
						if(l > len)
							len = l;
					}
				len += 2+1;
			...
			r1.max.y = r1.min.y+1;
			draw(i, r1, edgecolor, i->display->ones, r.min);
			r.min.y = r1.max.y;
		}
	}
	if(c->contents!=H)
