...
tkdrawlabel(Tk *tk, Point orig)
{
	...
	case TKradiobutton:
		...
		line(i, pp[0], pp[1], 0, 0, CheckButtonBW/2, cd, pp[0]);
		line(i, pp[1], pp[2], 0, 0, CheckButtonBW/2, cl, pp[1]);
		line(i, pp[2], pp[3], 0, 0, CheckButtonBW/2, cl, pp[2]);
		line(i, pp[3], pp[0], 0, 0, CheckButtonBW/2, cd, pp[3]);
		break;
	case TKcascade:
	...
	if(tkl->text != nil) {
		...
		if(tk->flag & Tkdisabled)
			col = TkCbackgnddark;
		tkdrawstring(tk, i, addpt(u, p), tkl->text, tkl->ul, col);
	}
