...
static void 	tktdrawline(Image*, Tk*, TkTline*, Point);
static void 	tktsetscroll(Tk*, int);
...
tktext(TkTop *t, char* arg, char **ret)
{
	...
	if(tkt->tagshare != nil)
		...
		for(i = 0; i < nelem(tktbinds); i++)
			tkaction(&tk->binds, tktbinds[i].event, TkStatic, tktbinds[i].cmd, TkAadd);
	}
...
/*if(tktdbg)print("q=(%d,%d)\n", q.x, q.y);*/
			...
			break;
		case TkTmark:
			if(strcmp(it->imark->name, "insert") == 0) {
				cursorx = p.x - 1;
			}
			break;
		case TkTwin:
 ...
 * Also, merge any adjacent TkTascii items with the same tags.
...
tktsetscroll(Tk *tk, int orient)
{
	...
	char val[Tkminitem], cmd[Tkminitem], *v, *e, *s;
	...
	*v++ = ' ';
	tkfprint(v, bot);
	sprint(cmd, "%s %s", s, val);
	e = tkexec(tk->env->top, cmd, nil);
	if(e != nil)
		print("tk: scrollcommand \"%s\": %s\n", cmd, e);
}
	...
	for(;;) {
		...
		case TkTwin:
			...
			if(sgml &&  sub != nil && sub->name != nil) {
				sprint(buf, "<Window %s>", sub->name->name);
				s = buf;
			}
		}
		if(s != nil) {
...
tktextbutton1(Tk *tk, char *arg, char **val)
{
	...
	c = tk->env->top->ctxt;
	if(!(tk->flag&Tkdisabled) && c->tkKgrab != tk && ix.item->kind != TkTwin) {
...
tktextcompare(Tk *tk, char *arg, char **val)
{
	...
	return tkvalue(val, tktindcompare(tkt, &i1, op, &i2)? "1" : "0");
}
...
tktextsearch(Tk *tk, char *arg, char **val)
{
	...
	while(!(ix1.item == ix2.item && ix1.pos == ix2.pos)) {
		if((ix1.item->kind == TkTascii || ix1.item->kind == TkTrune) &&
		    tktcmatch(ix1.item->istring[ix1.pos], *buf, nocase)) {
			if(ix1.item->kind == TkTrune) {
				n = chartorune(&r, ix1.item->istring);
				if(strncmp(ix1.item->istring, buf, n) != 0)
					goto nomatch;
				i = n;
			}
			else
			...
			for(; buf[i] != '\0'; i++) {
				...
				switch(tx.item->kind) {
				case TkTascii:
					c = tx.item->istring[tx.pos];
					break;
				case TkTrune:
					n = chartorune(&r, tx.item->istring);
					if(strncmp(tx.item->istring, buf+i, n) != 0)
						goto nomatch;
					i += n-1;
					goto contloop;
					break;
