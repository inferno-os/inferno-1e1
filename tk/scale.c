...
tkscalehoriz(Tk *tk, Image *i)
{
	...
	o.x += ScaleBW + (w*v)/TKF2I(tks->to-tks->from);
...
tkscalevert(Tk *tk, Image *i)
{
	...
	o.x += ScaleBW;
	v = TKF2I(tks->value-tks->from);
	o.y += ScaleBW + (h*v)/TKF2I(tks->to-tks->from);
...
tkscalecoords(Tk *tk, char *arg, char **val)
{
	...
	p = value*p/TKF2I(tks->to-tks->from);
	p += tks->pixmin;
	if(tks->orient == Tkvertical) {
		x = tks->center;
		y = p;
...
tkscaleget(Tk *tk, char *arg, char **val)
{
	...
	if(arg[0] != '\0') {
		...
		if(v > tks->pixmax)
			...
			value = v * ((tks->to-tks->from)/tks->pixmax-tks->pixmin);
			value += tks->from;
		}
		if(tks->res > 0)
