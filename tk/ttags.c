...
{
	"borderwidth",
		OPTdist, O(TkTtaginfo, opts[TkTborderwidth]),	nil,
	"justify",
	...
	"spacing1",
		OPTdist, O(TkTtaginfo, opts[TkTspacing1]),	IAUX(TKTEO),
	"spacing2",
		OPTdist, O(TkTtaginfo, opts[TkTspacing2]),	IAUX(TKTEO),
	"spacing3",
		OPTdist, O(TkTtaginfo, opts[TkTspacing3]),	IAUX(TKTEO),
	"offset",	
		OPTdist, O(TkTtaginfo, opts[TkToffset]),	IAUX(TKTEO),
	"underline",
		OPTdist, O(TkTtaginfo, opts[TkTunderline]),	nil,
	"overstrike",
		OPTdist, O(TkTtaginfo, opts[TkToverstrike]),	nil,
	"relief",
 ...
 */
void
tkttagopts(Tk *tk, TkTitem *it, int *opts, TkEnv *e, int dflt)
{
	int i;
	TkEnv *te;
	TkTtaginfo *tags;
...
tkttagbind(Tk *tk, char *arg, char **val)
{
	...
	if(cmd == nil)
		return TkNomem;
	tkaction(&ti->binds, event, TkDynamic, cmd, mode);
...
tkttagconfigure(Tk *tk, char *arg, char **val)
{
	...
	if(tkttagind(tk, ti->name, 1, &ix))
		tktfixgeom(tk, ix.line->prev, tkt->end.prev);
