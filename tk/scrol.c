...
TkEbind b[] = 
{
	...
	{TkButton1R,		"%W tkScrolBut1R; %W activate %x %y"},
};
...
tkhscroll(Tk *tk, TkScroll *tks, Image *i, Point size)
{
	...
	p[1].y = p[0].y - triangle/2;
	p[2].x = p[0].x + triangle;
	p[2].y = p[0].y + triangle/2;
...
tkscrollfraction(Tk *tk, char *arg, char **val)
{
	...
	frac = Tkfpscalar;
	if(len != 0)
		frac = (atoi(buf)-tks->a1)*Tkfpscalar/len;
	tkfprint(buf, frac);
	return tkvalue(val, "%s", frac);
}
...
tkScrollDrag(Tk *tk, char *arg, char **val)
{
	...
	if(tks->cmd != nil) {
		tkfprint(frac, tks->dragtop+delta);
		sprint(buf, "%s moveto %s", tks->cmd, frac);
