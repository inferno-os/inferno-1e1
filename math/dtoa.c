			...
			switch(c) {
			  ...
			  case 'N':
				if (match(&s, "an")) {
					word0(rv) = 0x7ff80000;
					word1(rv) = 0;
/* The value above should perhaps depend on the machine (to get a quiet
   NaN and avoid signalling, or perhaps we should just do arithmetic and
   let the INVAL or OVFL fall where they may. */
					goto ret;
					}
			  }
			s = s00;
