...
getXdata(Memimage *m, Rectangle r)
{
	...
	xi = XCreateImage(xdisplay, xvis, m->ldepth==3 ? xscreendepth : 1<<m->ldepth, ZPixmap, 0, (char*)p, Dx(m->r)+offset, Dy(m->r), 32, m->width*sizeof(ulong));
	 ...
	 * buffers we've seen, and Inferno's graphics code, requires big-endian
	 * bits within bytes.  Because the accesses are all bytewise, it we want
	...
	if(xscreendepth==24 && m->ldepth==3) {
		s = malloc(dx*sizeof(ulong)*dy);
...
getpmid0(Memimage *i)
{
	...
	memfillcolor(i0, 0); /* default memfillcolor for foreground is 0 */
	...
	pbase = (uchar*)(i->base + i->zero + i->r.min.y*i->width);
	p0base = (uchar*)(i0->base + i0->zero + i->r.min.y*i0->width);
	xmax = i->r.max.x;
	ymax = i->r.max.y;
...
if(0)print("hard draw state %d\n", state);
	...
	if(dst==src){
			...
			/* mask may be the same as src and dst, but it's silly and extremely rare */
			xmask = getXdata(mask, mmr);
	}else
	{	/* dst == mask */
	...
	if(xdst){
		XDestroyImage(xdst);
		dst->base = dstxm->wordp;
	}
	if(xsrc){
		XDestroyImage(xsrc);
		src->base = srcxm->wordp;
	}
	if(xmask){
		XDestroyImage(xmask);
		mask->base = maskxm->wordp;
	}
...
_memimagedraw(Memimage *dst, Rectangle r, Memimage *src, Point p0, Memimage *mask, Point p1, Rectangle sr)
{
	...
	case Simplemask|Replmask|Replsrc:
		d = byteaddr(dst, r.min);
		s = byteaddr(src, p0);
		if(src->base!=dst->base && src->ldepth==3)
			sparam.needbuf = 0;
	...
	case Simplesrc|Replsrc|Replmask:
		d = byteaddr(dst, r.min);
		m = byteaddr(mask, p1);
		mparam.needbuf = (mask->base == dst->base);
	...
	case Replsrc|Replmask:
		 ...
		 */
		mparam.needbuf = (mask->base==dst->base);
		sparam.needbuf = (src->base == dst->base);
		if(s > d){
			y0 = sr.min.y;
			y1 = sr.max.y;
			dy = 1;
