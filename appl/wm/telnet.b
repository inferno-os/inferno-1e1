...
{
	b := tk->toplevel(ctxt.screen, wmlib->geom(t)+" -borderwidth 2 -relief raised");

	titlectl := wmlib->titlebar(b, "Connect", 0);
	tklib->tkcmds(b, connect_cfg);
...
loop:	for(;;) alt {
	...
	wmlib->taskbar(b, "Dialing");
	tk->cmd(b, "update");

	ok: int;
	(ok, net) = sys->dial(addr, nil);
	if(ok < 0) {
		tklib->notice(b, "Connection to host failed\n"+sys->sprint("%r"));
		return;
	}

	wmlib->taskbar(t, "Telnet - "+addr);
	tklib->tkcmds(t, connected_cfg);
