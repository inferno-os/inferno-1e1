...
tkconfig := array[] of {
	...
	# search menu
	"menu .m.search.menu",
	".m.search.menu add command -state disabled -label Look -command {send m look}",
	".m.search.menu add command -state disabled -label {Search For} -command {send m search}",

	# program control
	"image create bitmap Detach -file "+icondir+"detach.bit -maskfile "+icondir+"detach.mask",
	"image create bitmap Kill -file "+icondir+"kill.bit -maskfile "+icondir+"kill.mask",
	"image create bitmap Run -file "+icondir+"run.bit -maskfile "+icondir+"run.mask",
	"image create bitmap Stop -file "+icondir+"stop.bit -maskfile "+icondir+"stop.mask",
	"image create bitmap Bpt -file "+icondir+"break.bit -maskfile "+icondir+"break.mask",
	"button .p.kill -image Kill -command {send m killall} -state disabled -relief sunken",
	"button .p.detach -image Detach -command {send m detach} -state disabled -relief sunken",
	"button .p.run -image Run -command {send m run} -state disabled -relief sunken",
	"frame .p.steps",
	"button .p.step -bitmap "+icondir+"opstep.bit -command {send m step} -state disabled -relief sunken",
	"button .p.stmt -bitmap "+icondir+"stepin.bit -command {send m stmt} -state disabled -relief sunken",
	"button .p.over -bitmap "+icondir+"stepsemi.bit -command {send m over} -state disabled -relief sunken",
	"button .p.out -bitmap "+icondir+"stepout.bit -command {send m out} -state disabled -relief sunken",
	"pack .p.step .p.stmt .p.over .p.out -in .p.steps -side top",
	"button .p.bpt -image Bpt -command {send m setbpt} -state disabled -relief sunken",
	"pack .p.kill .p.detach .p.run .p.steps .p.bpt -side top -pady 5",
	...
	# test body
	"frame .body.ft -bd 1 -relief sunken -width 60w",
	"scrollbar .body.scy",
...
init(ctxt: ref Draw->Context, argv: list of string)
{
	...
	if(tk == nil || tklib == nil || wmlib == nil || str == nil || readdir == nil
...
loop:	for(; argv != nil; argv = tl argv){
	...
	if(argc == 4 && hd argv == "-x" && hd tl tl argv == "-y")
		mygeom = "-x "+string(int hd tl argv + 20)+" -y "+string(int hd tl tl tl argv + 20);
	tktop = tk->toplevel(screen, mygeom+" -bd 2 -relief raised");
	...
	debsrc->init(screen, tktop, sys, tk, tklib, wmlib, str, debug);
	(datactl, datatitle) := debdata->init(screen, geom, debsrc, sys, tk, tklib, str, debug);

	title = sysnam+":Wmdeb";
	titlebut := wmlib->titlebar(tktop, title, Wmlib->Appl);
	...
	for(exiting := 0; !exiting || kids != nil; ){
		...
		s := <-m =>
			...
			"search" =>
				s = tklib->getstring(tktop, "Search For");
				if(s == ""){
...
open()
{
	file := wmlib->getfilename(screen, tktop, "Limbo source", "*.b", opendir);
	if(file == nil)
		return;
...
{
	t := tk->toplevel(screen, wmlib->geom(tktop)+" -borderwidth 2 -relief raised");
	cmd := chan of string;
	tk->namechan(t, cmd, "cmd");

	tklib->tkcmds(t, tkoptions);
	tabs := tklib->mktabs(t, ".opts", tkopttabs, oo.tabs);
	titlebut := wmlib->titlebar(t, "Wmdeb Options", wmlib->OK);
	tklib->tkcmds(t, tkoptpack);
...
pick:	for(;;){
		...
		alt{
		s := <-tabs =>
			o.tabs = tklib->tabsctl(t, ".opts", tkopttabs, o.tabs, s);
		s := <-titlebut =>
			case s{
			"exit" =>
				r <-= nil;
...
pickprog(c: chan of int)
{
	...
	titlebut := wmlib->titlebar(t, "Wmdeb Thread List", 0);
	tklib->tkcmds(t, tkpicktab);
	tk->cmd(t, "update");
	ids := addpickprogs(t);
...
kidstate()
{
	...
	if(kid == nil){
		...
		if(debsrc->packed == nil){
			tklib->tkcmds(tktop, searchoff);
			ts = tknobody;
		}else{
			ts = tkloaded;
			tklib->tkcmds(tktop, searchon);
		}
	}else{
