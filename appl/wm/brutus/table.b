...
init(asys: Sys, adraw: Draw, nil: Bufio, atk: Tk, atklib: Tklib)
{
	sys = asys;
	draw = adraw;
	tk = atk;
...
getspec(args: string) : (array of ref Lex, string)
{
	(n, argl) := sys->tokenize(args, " ");
	if(n != 1)
		return (nil, "usage: " + Name + " file");
	(filebytes, err) := readfile(hd argl);
	if(err != "")
		return (nil, err);
	return(html->lex(filebytes, 1), "");
}
...
cook(fmt: int, args: string) : (ref Celem, string)
{
	(spec, err) := getspec(args);
	if(err != "")
		return (nil, err);
	if(fmt == FLatex || fmt == FLatexProc || fmt == FLatexBook)
		return cooklatex(spec);
	else if(fmt == FHtml)
		return cookhtml(spec);
	return (nil, err_ret("unknown format"));
}
...
cooklatex(spec: array of ref Lex) : (ref Celem, string)
{
	...
	cur : ref Celem = nil;
	if(tab.capcell != nil)
		cur = add(ans, cur, specialce("\\begin{table}\n"));
	cur = add(ans, cur, specialce("\\begin{center}\n\\begin{tabular}{" + lcolspec() + "}\n"));
	if(tab.border) {
		if(tab.border == 1)
			s = "\\hline\n";
		else
	...
	if(tab.capcell != nil) {
		cur = add(ans, cur, specialce("\\caption{"));
		cur = addlconvlines(ans, cur, tab.capcell);
		cur = add(ans, cur, specialce("}\n\\end{table}\n"));
	}
