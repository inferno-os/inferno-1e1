...
include "draw.m";
draw: Draw;
Context, Display, Font, Screen, Image: import draw;
...
errmsg(title,msg: string)
{
	...
	le.append(Element.icon(env, noentry.r, noentry, display.ones));
	msg = "\n"+msg+"\n\n";
...
dialogue(expl: string, selection: list of string, width: int): int
{
	...
	title.append(Element.icon(env, regpic.r, regpic, display.ones));
...
verify(boxid: string, hispk, mypk, cert: array of byte): int
{
	...
	subtitle.append(Element.icon(env, regpic.r, regpic, display.ones));
