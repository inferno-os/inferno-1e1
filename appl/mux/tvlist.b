...
Ybox:		con 55;		# Size of Quarter box
Xbox:		con 60;
Xtitle:		con 90;		# Channel name
Shours:		con 1;
Xsep:		con 2;
Page:		con 6;
Brief:		con 0;
...
init(ctxt: ref Context, nil: list of string)
{
	...
	tistyle := ref *tvstyle;
	tistyle.textfont = Font.open(display, "/fonts/lucidasans/boldlatin1.8.font");
 	selstyle := ref *tvstyle;
#	selstyle.elemcolor = display.color(16ra9);
	selstyle.elemcolor = display.color(16r88);
	selstyle.textcolor = display.color(draw->White);
	selstyle.highlightcolor = display.color(draw->White);
...
detail(p: Progs, key: chan of int)
{
	...
	c := Compound.textbox(selenv, ((20, 20), zp), "", s);
	c.draw();
	<-key;		
}
...
getprog(channel: string, quarter: int): ref Chan
{
	...
	quarter *= 4;
	if(sys->seek(db, quarter, 0) != quarter)
		return c;
...
extract(prog, info: int): string
{
	...
	if(sys->seek(tmdata, prog, 0) != prog)
		return "Unknown";
