...
include "draw.m";
draw:	Draw;
Context, Display, Rect, Image, Font, Point, Screen: import draw;
...
init(ctxt: ref Context, nil: list of string)
{
	...
	stopch := chan of int;
	newi := display.newimage(((0,0),(2*R,H)), display.image.ldepth, 0, backrgb);
	neww = screen.newwindow(Rect((L,B), (R,B+H)), backrgb);
	...
	for (;;) {
		...
		alt {
		<- sourcech =>
			neww.draw(Rect((L,B), (R,B+H)), newi, display.ones,
				display.image.r.min);
...
timer(newi: ref Image, tch, stopch: chan of int)
{
	...
	for (;;) {
		...
		tch <- = 0 =>
			newi.draw(Rect((0,0),(2*R,H)), newi, display.ones,
			   newi.r.min.add((MOVE,0)));
			newi.draw(Rect((2*R-MOVE,0),(2*R,H)), backcol,
			    display.ones, newi.r.min);
			etext -= MOVE;
			w := textfont.width(s);
			neww.top();
			if (etext+w < 2*R) {
