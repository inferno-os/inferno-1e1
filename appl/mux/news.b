...
MENUFONT:	con "/fonts/lucidasans/news.10.font";
...
init(actxt: ref Context, nil: list of string)
{
	...
	softblue = display.color(16rD3);
	darkgreen = display.color(16rD9);
...
showpaper(p: ref Paper)
{
	...
	if(te.r.dy() > 300)
		te.clip(Rect(te.r.min,(te.r.max.x, te.r.min.y+300)));
	...
	headlines := Compound.box(headenv, Point(30,35), title, ee);
	headlines.draw();
...
finish()
{
	slavectl <-= Draw->AMexit;	# as good a value as any
...
outer:
	for(;;){
		...
		* =>		continue;
		...
		if (art==nil || art=="")
			continue;
		article = Compound.textbox(textenv, Rect((10, 60), (310, 460)), (hd t).title,
			art);
		article.draw();
