...
draw: Draw;
Context, Display, Point, Rect, Font, Image, Screen: import draw;
...
maketexture()
{
        ...
        texture = display.newimage(Rect((0, 0), (6, 6)), 3, 1, 16r80);
        gval := array[1] of { byte 255 };
        sval := array[1] of { byte 0 };
...
init(ctxt: ref Context,nil: list of string)
{
	...
	disp = screen.newwindow(BD, backblue);
	maketexture();
	disp.draw(FR, texture, ones, (0, 0));
