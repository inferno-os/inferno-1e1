...
include "draw.m";
draw: Draw;
Context, Display, Rect, Point, Image, Font, Screen: import draw;
...
init(ctxt: ref Context, nil: list of string)
{
	...
	tvstyle := ref Style(
			...
			display.color(16r87),		# titlecolor, purple-grey	
			display.color(16r38),	# textcolor, brown
			display.color(16r8a));	# highlightcolor, blue
	...
	te = Element.elist(tvenv, nil, Prefab->EHorizontal);
...
titles(cc: chan of int, cat: string)
{
	...
	for(t := l; t != nil; t = tl t)
		te.append(Element.text(menuenv, hd t, Rect((0, 0), (400, 0)),
				Prefab->EText));

	l = nil;

	te.adjust(Prefab->Adjpack, Prefab->Adjup);
	te.clip(Rect((0, 0), (400, 300)));
...
action(cc: chan of int, entry: ref Dbinfo, selelem: ref Element)
{
	...
	me := Element.text(menuenv, entry.info, Rect((0, 80), (600, 0)), Prefab->EText);
	...
	te.clip(Rect((0, 0), (600, te.r.max.y)));
	te.adjust(Prefab->Adjfill, Prefab->Adjcenter);
	me.append(Element.separator(menuenv, Rect((0, 0),(600,1)), ones, display.color(draw->Black)));
	me.append(te);
