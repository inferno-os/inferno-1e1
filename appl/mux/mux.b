...
include "draw.m";
draw: Draw;
Context, Display, Font, Image, Rect, Point, Pointer, Screen: import draw;
...
mainmenu(display: ref Display, screen: ref Screen): ref Compound
{
	...
	while(items != nil) {
		...
		ie:= Element.separator(env, ((0,0), (3,1)), display.zeros, display.zeros);
		te := Element.elist(env, ie, Prefab->EHorizontal);
		ie = Element.icon(env, icon.r, icon, display.ones);
		te.append(ie);
		ie = Element.separator(env, ((0,0), (5, 1)), display.zeros, display.zeros);
		te.append(ie);
...
loadptr(): int
{
	...
	if(mod!=nil && mod->init("/dev/pointer", ptrc)>=0)
...
newgroup(c: Mux, ctxt: ref Context, cmd: list of string, pin: int, pc: chan of int)
{
...
errmsg(title,msg: string, display: ref Display, screen: ref Screen, events: chan of int)
{
	...
	le.append(Element.icon(errenv, noentry.r, noentry, display.ones));
	msg = "\n"+msg+"\n\n";
