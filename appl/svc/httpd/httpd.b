...
usage(){
	sys->fprint(stderr, "usage: httpd [-c num -D ]\n");
	exit;
}
...
init(ctxt: ref Draw->Context, argv: list of string) {	
	...
	pars->init();
	my_domain=sysdom();	
	(ok, c) := sys->announce("tcp!*!http");
	if(ok < 0) {
		sys->fprint(stderr, "can't announce httpd: %r\n");
		exit;
	}
...
parsereq(g: ref Private_info) {
	...
	uri = pars->urlunesc(uri);
	uri = abspath(uri, SVR_ROOT);
	if(uri == nil || uri == "")
	...
	if(magic == nil || (magic == "httpd")){
		...
		if(uri==nil){
			...
			g.bout.puts(sys->sprint(
				"Your selection moved to <a href=\"%s\"> here</a>.<p>\r\n",
							 pars->urlconv(uri)));
			g.bout.flush();
		}
		atexit(g);
	...
	if(dir.mode & sys->CHDIR){
		...
		if(fd1 == nil){
			...
			g.bout.puts(sys->sprint("Location: http://%s%s\r\n", 
				pars->urlconv(g.host), pars->urlconv(w)));
			g.bout.puts("\r\n");
			g.bout.puts("<head><title>Object Moved</title></head>\r\n");
			g.bout.puts("<body><h1>Object Moved</h1></body>\r\n");
			g.bout.puts(sys->sprint(
...
stripmagic(uri : string): (string, string){
	prog,newuri : string;
	# we assume that uri begins /magic/
	if (!str->prefix("/magic/",uri))
		return(uri,nil);
	if (len newuri == 7)
		return(uri, nil);
	uri=uri[7:];
...
abspath(origpath,curdir : string): string {
	p, path, rpath : string;
	if(curdir == nil)
		curdir = "/";
	if(origpath == nil)
	...
	(path,nil) = str->splitl(path, "`;| ");
	if(path[0] == '/')
		rpath= "";
	else
		rpath = curdir;
	while(path!=nil){
		(p,path) = str->splitl(path, "/");
		if(p == "..")
			(rpath,nil)=str->splitr(rpath,"/");
		else if (p ==".")
			; #just strip it...
		else
			rpath+=p;
		if (len path > 1){
			path = path[1:];
			if (rpath==nil || rpath[len rpath-1]!='/')
				rpath[len rpath]='/';
		}
		if (path=="/"){
			rpath[len rpath]='/';
