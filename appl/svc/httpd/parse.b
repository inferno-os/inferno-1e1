...
init() {
	...
	if (date==nil){
		sys->print("date module load: %r\n");
		exit;
	}
	daytime = load Daytime Daytime->PATH;
	if(daytime == nil){
		sys->print("Daytime module load: %r\n");
		return;
	}
	cont = load Cont Cont->PATH;
	if(cont == nil){
		sys->print("cont module load: %r\n");
		return;
	}
	str = load String String->PATH;
	if(str == nil){
		sys->print("str module load: %r\n");
		return;
	}
	alm = load Alm Alm->PATH;
	if(alm == nil){
		sys->print("httpd alm module load: %r\n");
		return;
	}
	ch = chan of int;
	spawn lockproc();
...
atexit(g: ref Private_info){
	...
	if (g.bin!=nil)
		g.bufmod->g.bin.close();
	if (g.bout!=nil)
		g.bufmod->g.bout.close();
	g.bin=nil;
	g.bout=nil;
	exit;
}
...
mimeok(g: ref Private_info,name : string,multipart : int,head : list of ref Content): list of ref Content {
	...
	if(g.tok == '/' || multipart){
		...
		specific = "*";
	tmp := cont->mkcontent(generic, specific);
	head = tmp::head;
	for(;;){
		case g.tok {
		';' =>
...
parsejump(g: ref Private_info,k : string) {
	...
	"pragma" or "proxy-agent" or "proxy-connection" or "referer" or
...
fail(g: ref Private_info,reason : int, message : string){
	...
	body4:="</b><hr> This Webserver powered by <img src=\"/inf.gif\">. <P>"+
	...
	if (g.bout!=nil && reason!=2){
		g.bufmod->g.bout.puts(sys->sprint("%s %s\r\n", g.version, errormsg[reason].num));
		g.bufmod->g.bout.puts(sys->sprint("Date: %s\r\n", daytime->time()));
		g.bufmod->g.bout.puts(sys->sprint("Server: Charon\r\n"));
		...
		g.bufmod->g.bout.puts(sys->sprint("\r\n"));
		g.bufmod->g.bout.puts(strbuf);
		g.bufmod->g.bout.flush();
	}
...
okheaders(g: ref Private_info){
	g.bufmod->g.bout.puts(sys->sprint("%s 200 OK\r\n", g.version));
	g.bufmod->g.bout.puts("Server: Charon\r\n");
	g.bufmod->g.bout.puts("MIME-version: 1.0\r\n");
}

notmodified(g: ref Private_info){
	g.bufmod->g.bout.puts(sys->sprint("%s 304 Not Modified\r\n", g.version));
	g.bufmod->g.bout.puts("Server: Charon\r\n");
	g.bufmod->g.bout.puts("MIME-version: 1.0\r\n\r\n");
	atexit(g);
}
