...
read(fd: ref Iobuf): (ref Rawimage, string)
{
	...
	for(;;){
		...
		Bufio->EOF =>
			if(err == "")
				return (nil, "ReadGIF: premature EOF");
			break Loop;
		Bufio->ERROR =>
		...
		16r2C =>	# Image Descriptor
			if(image != nil){
				err = "ReadGIF: can't handle multiple images in file";
				break Loop;
			}
			(image, err) = readimage(header);
			if(image == nil)
				return (nil ,err);
			if(image.fields & 16r80){
				(image.cmap, err) = readcmap(header, (image.fields&7)+1);
				if(image.cmap == nil)
					return (nil, err);
			}else
				image.cmap = globalcmap;
			(image.chans[0], err) = decode(header, image);
			if(image.chans[0] == nil)
				return (nil, err);
			if(image.fields & 16r40)
				interlace(image);
			image.transp = header.transp;
			image.trindex = header.trindex;
