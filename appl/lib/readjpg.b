...
jfifheader(fd: ref Iobuf, ch: chan of (ref Rawimage, string)): (ref Header, string)
{
	buf := array[NBUF+1] of byte;
	if(fd.read(buf, 20) != 20)
		return (nil, sys->sprint("ReadJPG: can't read header: %r"));
	if(marker(buf, 0)!=SOI || marker(buf, 2)!=APPn || string buf[6:6+4]!="JFIF")
	...
	h.vers0 = int buf[11];
	h.vers1 = int buf[12];
	if(h.vers0>1 || h.vers1>2)
		return (nil, sys->sprint("ReadJPG: can't handle JFIF version %d.%2d", h.vers0, h.vers1));
	h.units = int buf[13];
	h.xdens = int2(buf, 14);
	h.ydens = int2(buf, 16);
	h.xthum = int buf[18];
	h.ythum = int buf[19];
	h.ch = ch;
	if(h.xthum!=0 || h.ythum!=0)
		return (h, sys->sprint("ReadJPG: can't handle thumbnails"));
	return (h, nil);
}
...
readsegment(h: ref Header): (int, array of byte, string)
{
	...
	n -= 2;
	if(n > len h.buf)
		return (-1, nil, "ReadJPG: segment too large");
	b := array[n] of byte;
	if(h.fd.read(b, n) != n)
		return (-1, nil, readerror());
	return (m, b, nil);
