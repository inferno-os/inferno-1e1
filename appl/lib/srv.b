...
auth(fd: ref Sys->FD): ref Sys->FD
{
	user := rf("/dev/user");
	ai := kr->readauthinfo("/usr/"+user+"/keyring/default");
	if(ai == nil){
		#call auth() or buffer will look like a bad message
		(idorerr, secret) := kr->auth(fd, ai, 1);
		sys->fprint(stderr, "readauthinfo failed: %r\n");
		return fd;
	}
	(idorerr, secret) := kr->auth(fd, ai, 1);
	if(secret == nil){
		sys->fprint(stderr, "authentication failed: %s\n", idorerr);
		return fd;
	}
	...
	algbuf := kr->getmsg(fd);
	if(algbuf == nil){
		sys->fprint(stderr, "authentication succeeded: %s\n", idorerr);
		return fd;
	}
	alg := string algbuf;
	if(alg == "none"){
	...
	if(ssl == nil){
		sys->fprint(stderr, "can't load ssl: %r\n");
		exit;
	}
	if(sys->bind("#D", "/n/ssl", Sys->MREPL) < 0){
		sys->fprint(stderr, "cannot bind #D: %r");
		exit;
	}
	(err, c) := ssl->connect(fd);
	if(c == nil){
		sys->fprint(stderr, "can't push ssl: %r\n");
		exit;
	}
	err = ssl->secret(c, secret, secret);
	if(err != nil){
		sys->fprint(stderr, "can't write secret: %s\n", err);
		exit;
	}
	if(sys->fprint(c.cfd, "alg %s", alg) < 0){
		sys->fprint(stderr, "can't push algorithm: %r\n");
		exit;
	}
	return c.dfd;
}
