...
init() : string {
	...
	tk = load Tk Tk->PATH;
	if (tklib==nil || wmlib==nil || str==nil || tk==nil)
		return "Not Initialised";
	# set up Draw context
	wmlib->init();
	tklib->init(tclmod.context);
	started=1;
	return nil;
}	
...
exec(tcl : ref Tcl_Core->TclData,argv : array of string) : (int,string) {
	...
	# so its a tk-command ... replace any -command with
...
do_toplevel(argv : array of string) : string {
	...
	whan=tk->toplevel((tclmod.context).screen,
				" -borderwidth 2 -relief raised");
	menubut := wmlib->titlebar(whan, argv[1],Wmlib->Appl);
	windows[i]=(argv[1],whan,menubut);
	if (tclmod.debug==1)
		sys->print("creating window %d, name %s, handle %ux\n",i,argv[1],whan);
	cmd := chan of string;
	tk->namechan(whan, cmd, argv[1]);
	tklib->tkcmds(whan, w_cfg);
	spawn menulisten(whan,menubut);
	return nil;
}


menulisten(t : ref Tk->Toplevel, menubut : chan of string) {
	for(;;) alt {
	menu := <-menubut =>
		if(menu[0] == 'e'){
			for(i:=0;i<len windows;i++){
