...
init(env : ref Draw->Context,d: ref Toplevel,box: int) : int
{
...
preamble(ioutb : ref Iobuf, bb: Rect) : int
{
	if (!started) return 1;
...
trailer(ioutb : ref Iobuf,pages : int) : int
{
	if (!started) return 1;
	ioutb.puts("%%Trailer\n%%Pages: "+string pages+"\n%%EOF\n");
	return 0;
...
printimage(ioutb : ref Iobuf,line : lineinfo, imag : iteminfo)
{
	RM:=612-18;
	#sys->print("Looking for [%s] of type [%s]\n",imag.buf,tk->cmd(t,"winfo class "+imag.buf));
	im := ctxt.display.open("/icons/tk/inferno.bit");
	if (line.xorg+imag.offset+imag.width>RM)
		imag.width=RM-line.xorg-imag.offset;
	ioutb.puts(sys->sprint("%d %d %d %d %d %d %d doimage\n",line.height,line.ascent,imag.width,
				line.yorg,line.xorg+imag.offset, im.r.dx(), im.r.dy()));
	imagebits(ioutb,im);
}
...
printline(ioutb: ref Iobuf,line : lineinfo,items : array of iteminfo)
{
	...
	ioutb.puts(sys->sprint("%d %d moveto\n",xstart+items[0].offset,
						line.yorg+line.height-line.ascent));
	for(j:=0;j<len items;j++){
		if (items[j].itype==IMAGE)
			printimage(ioutb,line,items[j]);
		else{
			if ((curfonttype&items[j].font)!=curfonttype){
				for(f:=0;f<curfont;f++){
					(ftype,fname)=font_arr[f];
					if ((ftype&items[j].font)==ftype)
						break;
				}
				if (f==curfont){
					fname=def_font;
...
parseTkline(ioutb: ref Iobuf,input : string) : int
{
	...
	thisline.xorg=(x*PXPI)/PTPI;
	(x,input)=str->toint(input,10);
	thisline.yorg=(x*PXPI)/PTPI;
	(x,input)=str->toint(input,10);
	thisline.width=(x*PXPI)/PTPI;
	(x,input)=str->toint(input,10);
	thisline.height=(x*PXPI)/PTPI;
	(x,input)=str->toint(input,10);
	thisline.ascent=(x*PXPI)/PTPI;
	(x,input)=str->toint(input,10);
	# thisline.numitems=x;
...
getline(k : int,  input : string) : array of iteminfo
{
	lineval,args : string;
	j, nb : int;
	lw:=0;
	...
	while(input!=nil){
		...
		case com {
			'A' =>
				...
				if (args!=nil){
					sys->print(
					"line %d item %d Bad Syntax args=%s\n",
							k,curitem,args);
					return nil;
				}
...
image2psfile(ioutb : ref Iobuf, im : ref Draw->Image, dpi: int) : int
{
	r := im.r;
	...
	xstart := 72;
	ystart := 720 - iheight;
	bbox := Rect(Point(xstart,ystart), Point(xstart+iwidth,ystart+iheight));
	if(preamble(ioutb, bbox) != 0)
		return ERROR;
	ioutb.puts("%%Page: 1\n%%BeginPageSetup\n");
	ioutb.puts("/pgsave save def\n");
	ioutb.puts("%%EndPageSetup\n");
	ioutb.puts(sys->sprint("%d 0 %d %d %d %d %d doimage\n", iheight, iwidth, ystart, xstart, height, width));
	imagebits(ioutb, im);
...
imagebits(ioutb : ref Iobuf, im : ref Draw->Image)
{
	...
	height:=im.r.dy();
	bps:=1<<im.ldepth;
	# sys->print("width=%d, height=%d, bits/pixel=%d\n",width,height,bps);
	arr:=array[(width*height*bps)/8] of byte;
	n:=im.readpixels(im.r,arr);
	# sys->print("%d bytes read\n",n);
	#To convert a pixel byte value v from a limbo image to
	#3 bytes (r,g,b), use:
	#	(r,g,b) := d.cmap2rgb(v);
	#where d is a display pointer (e.g., from context.display where
	#context is the init arg).
	lsf:=0;
	for(i:=0;i<n/4;i++){
		s:=cmap2ascii85(arr[4*i:4*(i+1)]);
		lsf+=len s;
		ioutb.puts(s);
		if (lsf>80){
		  ioutb.puts("\n");
