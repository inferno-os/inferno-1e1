...
open(ctxt: ref Draw->Context, title: string): ref Plot
{
	...
	tb := wmlib->titlebar(t, title, Wmlib->Appl);
	p := ref Plot(nil, Infinity,-Infinity,Infinity,-Infinity, textsize, t, tb, cc);
	TkCmd(p.t,"frame .fc");
	TkCmd(p.t,"frame .fc.b");
	TkCmd(p.t,"label .fc.b.xy -text {0 0} -anchor e");
	TkCmd(p.t,"pack .fc.b.xy -fill x");
	TkCmd(p.t,"pack .fc.b -fill both -expand 1");
	TkCmd(p.t,"canvas .fc.c -relief sunken -bd 2 -width 600 -height 480 -bg white -font /fonts/lucidasans/unicode.8.font");
	TkCmd(p.t,"pack .fc.c -fill both -expand 1");
	TkCmd(p.t,"pack .Wm_t -fill x");
	TkCmd(p.t,"pack .fc -fill both -expand 1");
	TkCmd(p.t,"pack propagate . 0");
	TkCmd(p.t,"bind .fc.c <ButtonPress-1> {send grcmd down1,%X,%Y}");
	return p;
}
...
Plot.bye(p: self ref Plot)
{
	...
	press := <-p.canvaschan =>
		...
		"down1" =>
			...
			x := (xpos-bx)/ax;
			y := -(ypos-tky+by-53.)/ay;  # fudge for titlebar and label
