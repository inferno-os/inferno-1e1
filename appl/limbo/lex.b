...
includef(file: ref Sym)
{
	...
	for(i := 0; b == nil && incpath[i] != nil && file.name[0] != '/'; i++){
...
lexcom(): int
{
	...
	for(s := 5; buf[s] == ' ' || buf[s] == '\t'; s++)
		;
	if((map[buf[s]] & Mdigit) == byte 0)
		return 0;
	n := 0;
	for(; (map[c = buf[s]] & Mdigit) != byte 0; s++)
		n = n * 10 + c - '0';
	for(; buf[s] == ' ' || buf[s] == '\t'; s++)
		;
	if(buf[s++] != '"')
...
lexid(c: int): int
{
	...
	for(;;){
		...
		c = getc();
		if(c == Bufio->EOF
		|| c < 256 && (map[c] & (Malpha|Mdigit)) == byte 0){
			ungetc(c);
			break;
		}
	}
...
lexnum(c: int): int
{
	Int, Radix, RadixSeen, Frac, ExpSeen, ExpSignSeen, Exp: con iota;
	...
	radix := "";
		...
		Int =>
			if((map[c] & Mdigit) != byte 0)
				break;
			if((map[c] & Mexp) != byte 0){
				state = ExpSeen;
				break;
			}
			if((map[c] & Mradix) != byte 0){
				radix = buf;
				buf = "";
				i = 0;
				state = RadixSeen;
...
escchar(): int
{
	...
	if(c == 'u'){
		...
		for(i := 0; i < 4; i++){
			c = getc();
			if(c == Bufio->EOF || (map[c] & (Mdigit|Mhex)) == byte 0){
...
loop:	for(;;){
	...
	yylval.tok.v.idval = enterstring(s);
}

lex(): int
{
	for(;;){
		yylval.tok.src.start = (lineno << PosBits) | (linepos & PosMask);
		case c := getc(){
		Bufio->EOF =>
			bin.close();
			if(bstack == 0)
		...
		'.' =>
			c = getc();
			ungetc(c);
			if(c != Bufio->EOF && (map[c] & Mdigit) != byte 0)
				return lexnum('.');
			return '.';
		'|' =>
			c = getc();
...
{
	t := lex();
	yylval.tok.src.stop = (lineno << PosBits) | (linepos & PosMask);
	return t;
}
