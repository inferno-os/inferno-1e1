...
cbra(Inst *i, int jmp)
{
	...
	if(UXSRC(i->add) == SRC(AIMM)) {
		...
		opwld(i, Ocmpw, RAX);
	genb(0x0f);
	rbra(patch[i->d.ins-mod->prog+1], jmp);
}
...
commcall(Inst *i)
{
	...
	modrm(Ostw, O(Frame, mr), RCX, RTA);	// MOVL R.TA, mr(CX) 	f->mr = R.M
	opwst(i, Oldw, RAX);			// MOVL ml, RAX
	modrm(Oldw, O(Modlink, m), RAX, RTA);	// MOVL ml->m, RTA
	modrm(Oldw, OA(Modlink, links)+i->reg*sizeof(Modl)+O(Modl, u.pc), RAX, RAX);
...
comp(Inst *i)
{
	...
	case ICALL:
		...
		/* no break */
	case IJMP:
		rbra(patch[i->d.ins-mod->prog+1], Ojmp);
		break;
	case IGOTO:
		opwst(i, Olea, RBX);
		opwld(i, Oldw, RAX);
...
macret(void)
{
	Inst i;
	uchar *s;
	static ulong lpunt, lnomr, lfrmr;
...
macmcal(void)
{
	...
	modrm(0x83, O(Module, prog), RTA, 7);	// CMPL $0, ml->m->prog
	genb(0x00);
	...
	*label = code-label-1;			// patch:
	...
	modrm(Ostw, O(REG, M), RTMP, RTA);	// MOVL RTA, R.M
	modrm(Oincrm, O(Module, ref), RTA, 0);	// INCL R.M->ref
	modrm(Oldw, O(Module, mp), RTA, RMP);	// MOVL R.M->mp, RMP
	modrm(Ostw, O(REG, MP), RTMP, RMP);	// MOVL RTA, R.MP	R.MP = ml->m
...
typecom(Type *t)
{
	...
	tmp = mallocz(4096*sizeof(char), 0);
	if(tmp == nil)
...
compile(Module *m, int size)
{
	...
	for(l = m->ext; l; l = l->next) {
		l->u.pc = (Inst*)(v+patch[l->u.pc-m->prog+1]);
		typecom(l->frame);
	}
