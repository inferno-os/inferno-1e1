...
static char	Ediffer[] = "images from different displays";
...
checkimage(Draw_Image *di)
{
	...
	i = lookupimage(di);
	if(lookupimage(di) == nil)
		error("image not allocated properly");
	return i;
}
...
checkscreen(Draw_Screen *ds)
{
	...
	s = lookupscreen(ds);
	if(s == nil)
		error("screen not allocated properly");
	return s;
}
...
checkfont(Draw_Font *df)
{
	...
	f = lookupfont(df);
	if(f == nil)
		error("font not allocated properly");
	return f;
}
...
checkdisplay(Draw_Display *dd)
{
	...
	d = lookupdisplay(dd);
	if(d == nil)
		error("display not allocated properly");
	return d;
}
...
Image_splinepoly(void *fp, int smooth)
{
	...
	if(smooth)
		bezspline(d, (Point*)f->p->data, f->p->len, f->end0, f->end1, f->radius, s, IPOINT(f->sp));
	else
		poly(d, (Point*)f->p->data, f->p->len, f->end0, f->end1, f->radius, s, IPOINT(f->sp));
	checkflush(f->dst);
	if(locked)
		unlockdisplay(d->display);
}
...
Image_bezier(void *fp)
{
	...
	if(d->display != s->display)
		error(Ediffer);
	if(f->radius < 0)
		error("negative bezier line width");
	locked = lockdisplay(d->display, 0);
	bezier(d, IPOINT(f->a), IPOINT(f->b), IPOINT(f->c), IPOINT(f->d), f->end0, f->end1, f->radius, s, IPOINT(f->sp));
	checkflush(f->dst);
	if(locked)
		unlockdisplay(d->display);
}
...
Image_fillbezier(void *fp)
{
	...
	if(d->display != s->display)
		error(Ediffer);
	locked = lockdisplay(d->display, 0);
	fillbezier(d, IPOINT(f->a), IPOINT(f->b), IPOINT(f->c), IPOINT(f->d), f->wind, s, IPOINT(f->sp));
	checkflush(f->dst);
	if(locked)
		unlockdisplay(d->display);
}
...
Image_fillsplinepoly(void *fp, int smooth)
{
	...
	/* sleazy: we know that Draw_Points have same shape as Points */
	if(smooth)
		fillbezspline(d, (Point*)f->p->data, f->p->len, f->wind, s, IPOINT(f->sp));
	else
		fillpoly(d, (Point*)f->p->data, f->p->len, f->wind, s, IPOINT(f->sp));
	checkflush(f->dst);
	if(locked)
		unlockdisplay(d->display);
}
...
arcellipse(void *fp, int isarc, int alpha, int phi)
{
	...
	if(f->a<0 || f->b<0)
		error("negative ellipse semidiameter");
	locked = lockdisplay(d->display, 0);
	if(isarc)
		arc(d, IPOINT(f->c), f->a, f->b, f->thick, s, IPOINT(f->sp), alpha, phi);
	else
		ellipse(d, IPOINT(f->c), f->a, f->b, f->thick, s, IPOINT(f->sp));
	checkflush(f->dst);
	if(locked)
		unlockdisplay(d->display);
}
...
Display_newimage(void *fp)
{
	...
	destroy(*f->ret);
	*f->ret = H;
	*f->ret = allocdrawimage((DDisplay*)f->d, f->r, f->ldepth, nil, f->repl, f->color);
	if(locked)
		unlockdisplay(d);
}
...
Display_open(void *fp)
{
	...
	*f->ret = H;
	...
	if(disp == nil)
		return;
	i = display_open(disp, stringchk(f->name));
	if(i == nil)
		return;
...
Display_writeimage(void *fp)
{
	...
	*f->ret = -1;
	...
	if(checkdisplay(f->d) != i->display)
		error("writeimage: image not on this display");
	...
	*f->ret = H;
	...
	if(font == nil) {
		...
		if(f->desc == H)
			...
			data = stringchk(f->desc);

		locked = lockdisplay(disp, 0);
		font = buildfont(disp, data, name, 0); /* BUG: 0 might not be right sometimes */
		if(locked)
			unlockdisplay(disp);
		if(font == nil)
			return;
...
Font_open(void *fp)
{
	...
	*f->ret = H;
	...
	font = font_open(disp, stringchk(f->name));
	if(font == 0)
		return;
	...
	for(i=0; i<array->len; i++)
		if(di[i] != H){
			...
			if(ip[n]==nil || ip[n]->screen != s){
				free(ip);
				error("window not part of screen in Screen.top");
			}
			n++;
		}
	if(n == 0){
...
doflush(Display *d)
{
	...
	if(kchanio(d->datachan, d->buf, n, OWRITE) != n){
		...
		err[0] = 0;
		kerrstr(err);
		fprint(2, "flushimage fail: d=%lux: %s\n", d, err);
...
delrefresh(Image *i)
{
	 ...
	 */
	if(d->local)
		drawlsetrefresh(d->dataqid, i->id, localrefreshcompound, nil);
	refptr = i->refptr;
	i->refptr = nil;
