 ...
 *	bounds checks
 *	module pointer checks
 *	enable and check FP code when ericvh sends l.s & kernel changes
 *	(i think the saved register set doesn't match ureg.h for fpemu)
 */
...
commframe(Inst *i)
{
	...
	mem(Ldw, (ulong)&((Modlink*)0)->links[i->reg].frame, RA0, RA3);
...
//	if(t->np<<5 >= 0xFFF) long offsets ... adjust RA2
