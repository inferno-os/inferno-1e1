...
rqsort(int *a, int n, double *x)
{
	...
	r = min(pd-pc, pn-pd-1); vecswap(pb, pn-r, r);
...
Math_dot(void *fp)
{
	...
	f = fp;
	if(f->x->len!=f->y->len){
		print("dot: incompatible lengths\n");
		return;
	}
...
Math_gemm(void *fp)
{
	...
	if(f->a != H){
		...
		if(ld<nrowa || ld*(ncola-1)>mn)
			error("gemm: bounds exceeded for a");
	}
	if(f->b != H){
		mn = f->b->len;
		ld = f->ldb;
		...
		if(ld<nrowb || ld*(ncolb-1)>mn)
			error("gemm: bounds exceeded for b");
	}
	m = f->m;
	n = f->n;
	mn = f->c->len;
	...
	ld = f->ldc;
	if(ld<m || ld*(n-1)>mn)
		error("gemm: bounds exceeded for c");
