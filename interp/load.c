...
load(char *path)
{
	...
	m = nil;
	code = readmod(path, nil, &mtime, 0);
	if(code != nil) {
		m = parsemod(path, code, mtime);
		free(code);
	}
	return m;
}
...
Type*
dtype(void (*destroy)(Heap*), int size, uchar *map, int mapsize)
{
	Type *t;
...
brpatch(Inst *ip, Module *m)
{
	...
	case IBGTL:
	case IBGEL:
		ip->d.imm = (WORD)&m->prog[ip->d.imm-1];
		break;
	case ISPAWN:
...
Module*
parsemod(char *path, uchar *code, ulong mtime)
{
	Heap *h;
	Inst *ip;
	Type *pt;
