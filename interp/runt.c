			...
			*f = '\0';
			...
			case 's':
				...
				if(ss->len < 0) {
					f[-1] = 'S';
					ss->Srune[-ss->len] = L'\0';
					b += snprint(b, eb-b, fmt, ss->Srune);
				}
				else {
					ss->Sascii[ss->len] = '\0';
					b += snprint(b, eb-b, fmt, ss->Sascii);
				}
				break;
			case 'e':
			case 'f':
...
int
utfnlen(char *s, int nb, int *ngood)
{
	int c;
	long n;
	Rune rune;
