...
comp(Inst *i)
{
	...
	braw:
		opwld(i, Oldw, RA0);
		mid(i, Oldw, RA1);
		combdis(RA0, r, RA1, f, i->d.ins-mod->prog+1);
		nop();
		break;		
	case IBNEW:
		r = Cequal;
...
macmcal(void)
{
	...
	mem(Oldw, O(Module, mp), RA3, RMP);
	BV(RA0);
	mem(Ostw, O(REG, MP), RREG, RMP);	// (delay)
}
