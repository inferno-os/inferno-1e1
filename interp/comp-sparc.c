...
#define PC30(pc)		   (((ulong)(pc) - (ulong)code + 4)>>2)

#define CALL(addr)		   *code++=OP(1)|PC30(addr)
#define FM2I(op2, i, rd)	   *code++=OP(0)|(rd<<25)|(op2<<22)|D22(i)
#define BRA(cond, disp)		   *code++=OP(0)|(cond<<25)|(2<<22)|D22((disp))
#define BRAF(cond, disp)	   *code++=OP(0)|(cond<<25)|(6<<22)|D22((disp))
#define BRADIS(r, o)		   BRA(r, ((ulong)(base+patch[o]+1)-(ulong)code)>>2)
#define BRAFDIS(r, o)		   BRAF(r, ((ulong)(base+patch[o]+1)-(ulong)code)>>2)
#define BRAMAC(r, o)		   BRA(r, ((ulong)(base+macro[o]+1)-(ulong)code)>>2);
...
commcall(Inst *i)
{
	...
	opwst(i, Oldw, RA0);
	mem(Oldw, O(Modlink, m), RA0, RA3);
	CALL(base+macro[MacMCAL]);
	mem(Oldw, OA(Modlink, links)+i->reg*sizeof(Modl)+O(Modl, u.pc), RA0, RA0);
}
...
macret(void)
{
	...
	mem(Oldw, O(Module,ref),RA2, RA3);
	FM3I(2, Oaddcc, -1, RA3, RA3);
	cp5 = code;
	BRA(Obe, 0);				// --ref(arg) == 0
	NOOP;
	mem(Ostw, O(Module,ref),RA2, RA3);

	mem(Oldw, O(Frame,mr),RFP, RA1);
	mem(Ostw, O(REG,M),RREG, RA1);
	mem(Oldw, O(Module,mp),RA1, RMP);
...
macmcal(void)
{
	...
	mem(Oldw, O(Module, prog), RA3, RA1);
	FM3I(2, Ocmp, 0, RA1, RZ);
	lab = code;
	BRA(Obne, 0);
	NOOP;
	...
	mem(Oldw, O(Module, ref), RA3, RA1);
	FM3I(2, Oadd, 1, RA1, RA1);
	mem(Ostw, O(Module, ref), RA3, RA1);
	mem(Oldw, O(Module, mp), RA3, RMP);	// MOVL R.M->mp, RMP
	FM3I(2, Ojmpl, 0, RA0, RZ);
	mem(Ostw, O(REG, MP), RREG, RMP);	// MOVL RA3, R.MP	R.MP = ml->m
}
...
compile(Module *m, int size)
{
	...
	if(cflag > 1)
		print("dis=%5d %5d sparc=%5d asm=%.8lux lit=%d: %s\n",
