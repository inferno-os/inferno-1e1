...
branch(Inst *i)
{
	...
	if(base == 0)
		return 0;
	rel = patch[(Inst*)i->d.imm - mod->prog + 1];
	rel += (base - code) - 1;
	return rel & 0xffff;
}
...
commcall(Inst *i)
{
	...
	op3(i, Olw, Ro2, 1);
	IRR(Olw, O(Modlink,m),Ro2, Ri);			// ml->m in Ri
	ml = nil;
	IRR(Olw, (ulong)&ml->links[i->reg].u.pc,Ro2, Rj);// ml->entry in Rj
...
comp(Inst *i)
{
	...
	case ICALL:
		...
		J(Oj, base+patch[(Inst*)i->d.imm - mod->prog + 1]);
...
// NOTE this needs to be scheduled
	...
	delay();
	IRR(Olw, O(Module,ref),Ro2, Ro3);
	delay();
	IRR(Oaddui, -1,Ro3, Ro3);
	cp5 = code;
	BRRI(Obeq, Ro3,Rzero, 0);		// --ref(arg) == 0
	delay();
	IRR(Osw, O(Module,ref),Ro2, Ro3);

	IRR(Olw, O(Frame,mr),Rfp, Ro1);
	delay();
	IRR(Osw, O(REG,M),Rreg, Ro1);
	IRR(Olw, O(Module,mp),Ro1, Rmp);
	delay();
...
macmcal(void)
{
	...
	*cp |= (code - cp) - 1;
	IRR(Olw, O(Module,ref),Ri, Ro2);
	IRR(Osw, O(REG,M),Rreg, Ri);
	IRR(Oaddui, 1,Ro2, Ro2);
	IRR(Olw, O(Module,mp),Ri, Rmp);
	IRR(Osw, O(Module,ref),Ri, Ro2);
	IRR(Osw, O(REG,MP),Rreg, Rmp);
...
compile(Module *m, int size)
{
	...
	if(cflag > 1)
		print("dis=%5d %5d mips=%5d asm=%.8lux lit=%d: %s\n",
