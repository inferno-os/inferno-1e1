...
char Tokalloc[]	= "adt not allocated properly";
char Tkind[]	= "bad kind";

void	freeprefabcompound(Heap*);
...
prefabmodinit(void)
{
	...
	builtinmod("$Prefab", Prefabmodtab);
}
...
findtag(PElement *pelem, char *tag)
{
	...
	for(l=pelem->first; l!=H; l=l->tail){
		pe = *(PElement**)l->data;
		if(strcmp(tag, stringchk(pe->e.tag)) == 0)
			return pe;
...
Element_iconseparator(void *fp, int kind)
{
	...
	f = fp;
	badenviron(f->env, 1);
	checkimage(f->mask);	/* error check only */
	icon = checkimage(f->icon);
	locked = lockdisplay(icon->display, 0);
	destroy(*f->ret);
	*f->ret = H;
	if(kind == ESeparator)
...
Compound_box(void *fp)
{
	...
	badenviron(f->env, 1);
	if(f->title != H)
		checkelement(f->title);	/* error check only */
	checkelement(f->elist);	/* error check only */
	disp = checkscreen(f->env->screen)->display;
	locked = lockdisplay(disp, 0);
	destroy(*f->ret);
	*f->ret = H;
...
Compound_draw(void *fp)
{
	...
	locked = lockdisplay(pc->display, 0);
	drawcompound(&pc->c);
...
pelement(Prefab_Compound *comp, Prefab_Element *elem)
{
	...
	if(comp == H)
		return H;
	checkcompound(comp);	/* error check */
