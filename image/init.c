...
initdisplay(char *dev)
{
	...
	sprint(buf, "%s/new", dev);
	ctlfd = libopen(buf, ORDWR);
	if(ctlfd < 0){
		if(libbind("#d", dev, MAFTER) < 0){
    Error1:
			libqlfree(q);
			kwerrstr("initdisplay: %r");
			return 0;
		}
		ctlfd = libopen(buf, ORDWR);
	}
	...
	sprint(buf, "%s/%d/data", dev, atoi(info+0*12));
	datafd = libopen(buf, ORDWR);
	if(datafd < 0)
		goto Error2;
	sprint(buf, "%s/%d/refresh", dev, atoi(info+0*12));
	...
	disp->qlock = q;
	libqlock(q);
	disp->ones = allocimage(disp, Rect(0, 0, 1, 1), 3, 1, 0xFF);
	disp->zeros = allocimage(disp, Rect(0, 0, 1, 1), 3, 1, 0x00);
