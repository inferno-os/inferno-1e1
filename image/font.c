...
static Subfont*
cf2subfont(Cachefont *cf, Font *f, int ldepth)
{
	char *name;
	Subfont *sf;
	...
	name = cf->subfontname;
	if(name == nil){
		name = subfontname(cf->name, f->name, ldepth);
		if(name == nil)
			return nil;
		cf->subfontname = name;
	}
...
loadchar(Font *f, Rune r, Cacheinfo *c, int h, int noflush, char **subfontname)
{
	...
	subf->age = 0;
	subf->cf = nil;
	subf->f = cf2subfont(cf, f, f->ldepth);
	if(subf->f == nil){
		*subfontname = cf->subfontname;
		return -1;
	}

	if(subf->f->bits->ldepth > f->maxldepth)
		f->maxldepth = subf->f->bits->ldepth;
	subf->cf = cf;
	if(subf->f->ascent > f->ascent){
		...
		b = subf->f->bits;
		draw(b, b->r, b, f->display->ones, addpt(b->r.min, Pt(0, d)));
		draw(b, Rect(b->r.min.x, b->r.max.y-d, b->r.max.x, b->r.max.y), f->display->zeros, f->display->ones, b->r.min);
...
resize(Font *f, int wid, int ncache)
{
	...
	ret = 0;
	d = f->display;
	new = allocimage(d, Rect(0, 0, ncache*wid, f->height), f->ldepth, 0, 0);
	if(new){
		b = bufimage(d, 10);
		if(b == 0){
			freeimage(new);
