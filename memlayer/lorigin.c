...
memlorigin(Memimage *i, Point log, Point scr)
{
	...
	if(eqlog==0 && l->save!=nil){
		nsave = allocmemimage(Rect(log.x, log.y, log.x+Dx(oldr), log.y+Dy(oldr)), i->ldepth);
		if(nsave == nil)
			return -1;
	}
	...
	if(nsave){
		if(!wasclear)
			memimagedraw(nsave, nsave->r, l->save, l->save->r.min, memones, Pt(0,0));
		freememimage(l->save);
		l->save = nsave;
	}
	 ...
	 */
	shad = memlalloc(s, oldr, memlnorefresh, nil, -1);
	s->frontmost = i;
	if(s->rearmost == i)
		s->rearmost = shad;
	else
	 ...
	 */
	if(wasclear)
		memdraw(l->screen->image, newr, s->image, oldr.min, memones, Pt(0,0));
	else
		memlexpose(i, newr);
	memldelete(shad);
