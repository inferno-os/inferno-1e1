...
{
	int fd, cfd, n;
	char name[3*NAMELEN+5], data[3*NAMELEN+10], err[ERRLEN], *p;

	cfd = kopen(clone, ORDWR);
	if(cfd < 0){
		kerrstr(err);
		kwerrstr("open %s: %s", clone, err);
		return -1;
	}
